...
> Upstream path: `document/4-Web_Application_Security_Testing/07-Input_Validation_Testing/05.3-Testing_for_SQL_Server.md`  
> Upstream Git blob: `2ab27b5cc992a20b8b39186bc1a74738c5536218`  
...
- Useful stored procedures and features include:
    - [xp_cmdshell](https://learn.microsoft.com/en-us/sql/relational-databases/system-stored-procedures/xp-cmdshell-transact-sql) executes a command shell on the server under the SQL Server service account context. It requires `sysadmin` privileges and has been disabled by default since SQL Server 2005 (it can be re-enabled with `sp_configure` when advanced options are shown).
    ...
    - [sp_makewebtask](https://learn.microsoft.com/en-us/previous-versions/sql/sql-server-2008/ms180099(v=sql.100)) (removed; present only on very old SQL Server versions) wrote query results to a file via the Web Assistant.
    - [xp_sendmail](https://learn.microsoft.com/en-us/previous-versions/sql/sql-server-2008-r2/ms189505(v=sql.105)) (removed with SQL Mail) sent email that could include a query result set attachment.

The examples below focus on techniques that remain relevant on SQL Server and Azure SQL Managed Instance when privileged features are exposed. Most use `exec`.
...
On legacy SQL Server versions that still provide Web Assistant procedures, `sp_makewebtask` could be used instead:

`exec sp_makewebtask 'C:\Inetpub\wwwroot\test.txt', 'select * from master.dbo.sysobjects'--`

A successful execution will create a file that can be browsed by the tester.
...
Notice the use of [convert](https://learn.microsoft.com/en-us/sql/t-sql/functions/cast-and-convert-transact-sql):
...
### Example 5: Enabling or Replacing `xp_cmdshell`

Security baselines keep `xp_cmdshell` disabled. With `sysadmin` rights (natively or after recovering the `sa` or admin password; see below), re-enable it or recreate equivalent functionality.

Enable `xp_cmdshell` by injecting:
...
On legacy SQL Server 2000 estates, if `xp_cmdshell` was removed with `sp_dropextendedproc`, it could be restored with:

`sp_addextendedproc 'xp_cmdshell','xp_log70.dll'`

If that failed because `xp_log70.dll` was moved or deleted, a custom procedure using OLE Automation (`sp_OACreate`, `sp_OAMethod`, `sp_OADestroy`) could recreate shell execution, as described by Antonin Foller (see links at the bottom of the page), provided OLE Automation remained enabled.
...
Keep in mind that `OPENROWSET` and Ad Hoc Distributed Queries have been disabled by default since SQL Server 2005 (they were enabled by default on SQL Server 2000).
...
The same timing approach can be used to identify the SQL Server version via the built-in `@@version` variable:
...
Because the product year and build number sit at different offsets across releases, extract a distinctive substring (for example `2022`, `2019`, or the major version number such as `16.0`) rather than hard-coding a single character position. A timing-based check might look like:

`if (select @@version) like '%2022%' waitfor delay '0:0:5'`
...
Remember that Ad Hoc Distributed Queries and `OPENROWSET` have been restricted since SQL Server 2005 and must usually be enabled explicitly by an administrator.
