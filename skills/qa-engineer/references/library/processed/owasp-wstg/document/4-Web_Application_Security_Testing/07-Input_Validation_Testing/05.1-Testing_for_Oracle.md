...
> Upstream path: `document/4-Web_Application_Security_Testing/07-Input_Validation_Testing/05.1-Testing_for_Oracle.md`  
> Upstream Git blob: `784a6a3d87e599a506f8d90f23e0b7c5f67117e8`  
...
Web-based PL/SQL applications are enabled by the PL/SQL Gateway, which translates web requests into database queries. Many deployments expose this through [Oracle REST Data Services (ORDS)](https://docs.oracle.com/en/database/oracle/oracle-rest-data-services/26.1/orddg/introduction-to-Oracle-REST-Data-Services.html), often fronting [Oracle APEX](https://docs.oracle.com/en/database/oracle/apex/). Older estates may still use Apache `mod_plsql`, Oracle HTTP Server, Oracle Application Server, Portal, HTML DB / APEX predecessors, WebDB, or the XML Database (XDB) web server.

This chapter focuses on recognizing and testing PL/SQL Gateway endpoints, including legacy `mod_plsql` and Application Server deployments that remain in production. Apply the same input-validation mindset to ORDS and APEX surfaces, and use the general [SQL Injection](05-Testing_for_SQL_Injection.md) guidance for Oracle-backed applications that do not expose the gateway.

## How to Test

### How the PL/SQL Gateway Works

Essentially the PL/SQL Gateway acts as a proxy server taking the user's web request and passes it on to the database server where it is executed.
...
The second and third of these examples represent URLs from older PL/SQL Gateway deployments; the first is typical of Apache `mod_plsql` (`/pls` is the default Location in `plsql.conf`, though the path can be changed). ORDS mappings differ by configuration and may not use `/pls`. The absence of a file extension in a URL could indicate the presence of the Oracle PL/SQL Gateway. Consider the following URL:
...
In this URL, xyz is the Database Access Descriptor, or DAD. A DAD specifies information about the database server so that the PL/SQL Gateway can connect. It contains information such as the TNS connect string, the user ID and password, and authentication methods. On Apache `mod_plsql` these DADs are specified in `dads.conf`; older listeners used `wdbsvr.app`. ORDS uses its own pool/configuration files instead of DADs. Some historical default DAD names include the following:
...
Response headers can help identify Oracle web tiers, but they are neither required nor reliable on their own. Many ORDS and APEX deployments sit behind reverse proxies or content delivery networks (CDNs) and return a generic `Server` value, or none. Prefer URL shape and the probes below when banners are absent or rewritten.

Useful signals when present:

- `Oracle-REST-Data-Services/...` (ORDS).
- `Oracle-HTTP-Server`, `mod_plsql`, or `Oracle-Application-Server` (legacy Oracle HTTP Server or Application Server).
- Related headers such as `X-ORDS-*` or older `X-APEX-*`.

Examples:

```text
Server: Oracle-REST-Data-Services/24.2.0.r1234567
Server: Oracle-Application-Server-10g/10.1.2.0.0 Oracle-HTTP-Server
Server: Oracle HTTP Server Powered by Apache/1.3.22 (Unix) mod_plsql/9.0.2.0.0
```
...
Simply placing the name of the schema/package in double quotes could allow an attacker to bypass the exclusion list. Some Oracle Application Server releases (notably 10g) convert the user's request to lowercase before sending it to the database server; because a quoted identifier is case sensitive, `SYS` and `sys` are not the same and requests for the latter will result in a 404 Not Found. On earlier versions the following can bypass the exclusion list:
...
Everything after the double minus (`--`) is treated as a comment. This request will cause an internal server error because one of the bind variables is no longer used, so the attacker needs to add it back. As it happens, it’s this bind variable that is the key to running arbitrary PL/SQL. For the moment, they can just use `HTP.PRINT` to print BAR, and add the needed bind variable as :1:
...
> Note: the output won’t be displayed. This can be leveraged to exploit any PL/SQL injection bugs owned by SYS, thus enabling an attacker to gain complete control of the backend database server. For example, the following URL takes advantage of the SQL injection flaws in `DBMS_EXPORT_EXTENSION`
...
- [Introduction to Oracle REST Data Services](https://docs.oracle.com/en/database/oracle/oracle-rest-data-services/26.1/orddg/introduction-to-Oracle-REST-Data-Services.html)
- [ORDS PL/SQL Gateway](https://docs.oracle.com/en/database/oracle/oracle-rest-data-services/26.1/orddg/pl-sql-gateway.html)
- [APEX Web Server Security Considerations](https://docs.oracle.com/en/database/oracle/apex/26.1/htmdb/web-server-security-considerations.html)
- [SQL Injection (Oracle Database PL/SQL Language Reference)](https://docs.oracle.com/en/database/oracle/oracle-database/26/lnpls/sql-injection.html)
