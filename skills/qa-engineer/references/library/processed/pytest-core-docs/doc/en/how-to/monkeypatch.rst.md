...
> Upstream path: `doc/en/how-to/monkeypatch.rst`  
> Upstream Git blob: `642e965b618d61304d3745c679877a7050c3f42c`  
...
- :meth:`monkeypatch.syspath\_prepend(path) <pytest.MonkeyPatch.syspath\_prepend>`
...
5. Use :py:meth:`monkeypatch.syspath\_prepend <MonkeyPatch.syspath\_prepend>` to modify sys.path which will also
call pkg\_resources.fixup\_namespace\_packages and :py:func:`importlib.invalidate\_caches`.
...
See the [monkeypatch blog post](https://tetamap.wordpress.com//2009/03/03/monkeypatching-in-unit-tests-done-right/) for some introduction material
...
monkeypatch applies the mock for requests.get with our mock\_get function.
The mock\_get function returns an instance of the MockResponse class, which
...
For testing purposes we can patch the DEFAULT\_CONFIG dictionary to specific values.
