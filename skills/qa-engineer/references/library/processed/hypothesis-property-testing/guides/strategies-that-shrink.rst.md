...
> Upstream path: `guides/strategies-that-shrink.rst`  
> Upstream Git blob: `a49d4626b16fac0c3aeabb9c1869eae7318d89df`  
...
as [hypothesis-networkx](https://pypi.org/project/hypothesis-networkx/)),
...
[Issue #1076](https://github.com/HypothesisWorks/hypothesis/issues/1076),
where magnitude constraints were added to the complex\_numbers strategy,
...
between +/- max\_magnitude, then calculate the resulting bounds on the real
...
[Issue #69](https://github.com/HypothesisWorks/hypothesis/issues/69) provides
...
and it \_works\_, but it's often much slower than the alternatives.
...
the [internals guide](https://github.com/HypothesisWorks/hypothesis/blob/master/guides/internals.rst)
...
buffer. We use it for sampled\_from(...).filter(...), after trying an
initial draw with the usual rejection sampling technique, and added the
SearchStrategy.do\_filtered\_draw method so other strategies can opt-in
...
An important insight from [Swarm Testing (PDF)](https://www.cs.utah.edu/~regehr/papers/swarm12.pdf)
...
As discussed on [issue #1401](https://github.com/HypothesisWorks/hypothesis/issues/1401),
...
2. Use characters().filter(category\_is\_not\_banned)
...
deleted simultaneously using data.start\_span. This is used to group
...
draws, such as our internal helper cu.biased\_coin, which makes eliminating
