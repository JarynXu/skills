...
> Upstream path: `src/main/docs/guide/config/configurationProperties.adoc`  
> Upstream Git blob: `39683df65243717e40697052860aa98baf82cf69`  
...
Configuration values can then be supplied from one of the [PropertySource]({api}/io/micronaut/context/env/PropertySource.html) instances. For example:
...
This is a limitation of YAML/SnakeYAML parsing and not specific to Micronaut. For more information, see the [YAML 1.2 Specification](https://yaml.org/spec/1.2/spec.html#id2765879).
====
...
Since 3.3, the Micronaut framework supports defining different accessors prefixes for getters and setter other than the default `get` and `set` defined for Java Beans. Annotate your POJO or `@ConfigurationProperties` class with the [@AccessorsStyle]({api}/io/micronaut/core/annotation/AccessorsStyle.html) annotation.
...
You can use the [@ConfigurationBuilder]({api}/io/micronaut/context/annotation/ConfigurationBuilder.html) annotation to populate a builder-style class with configuration values. [ConfigurationBuilder]({api}/io/micronaut/context/annotation/ConfigurationBuilder.html) can be applied to fields or methods in a class annotated with [@ConfigurationProperties]({api}/io/micronaut/context/annotation/ConfigurationProperties.html).
...
A configuration prefix can also be specified to tell the Micronaut framework where to look for configuration values. By default, builder methods use the configuration prefix specified in a class-level [@ConfigurationProperties]({api}/io/micronaut/context/annotation/ConfigurationProperties.html) annotation.
...
Configuration values can be supplied from one of the [PropertySource]({api}/io/micronaut/context/env/PropertySource.html) instances. For example:
