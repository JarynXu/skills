...
> Upstream path: `src/main/docs/guide/httpServer/binding.adoc`  
> Upstream Git blob: `abbcdc2cdf1843f49eff55b350842cc3c0a20916`  
...
|[@Body]({api}/io/micronaut/http/annotation/Body.html)
|Binds from the body of the request
|`@Body String body`

|[@CookieValue]({api}/io/micronaut/http/annotation/CookieValue.html)
|Binds a parameter from a cookie
|`@CookieValue String myCookie`

|[@Header]({api}/io/micronaut/http/annotation/Header.html)
|Binds a parameter from an HTTP header
|`@Header String requestId`

|[@QueryValue]({api}/io/micronaut/http/annotation/QueryValue.html)
|Binds from a request query parameter
|`@QueryValue String myParam`

|[@Part]({api}/io/micronaut/http/annotation/Part.html)
|Binds from a part of a multipart request
|`@Part CompletedFileUpload file`

|[@RequestAttribute]({api}/io/micronaut/http/annotation/RequestAttribute.html)
|Binds from an attribute of the request. Attributes are typically created in filters
|`@RequestAttribute String myAttribute`

|[@PathVariable]({api}/io/micronaut/http/annotation/PathVariable.html)
|Binds from the path of the request
|`@PathVariable String id`

|[@RequestBean]({api}/io/micronaut/http/annotation/RequestBean.html)
...
Generally any type that can be converted from a String representation to a Java type via the [ConversionService]({api}/io/micronaut/core/convert/ConversionService.html) API can be bound to.

This includes most common Java types. However, you can simply add additional [TypeConverter]({api}/io/micronaut/core/convert/TypeConverter.html) either by defining it as a bean or by registering it in a api:core.convert.TypeConverterRegistrar[] via the service loader.
...
Additionally, any `DateTime` that conforms to [RFC-1123]({jdkapi}/java.base/java/time/format/DateTimeFormatter.html#RFC_1123_DATE_TIME) can be bound to a parameter. Alternatively the format can be customized with the [Format]({api}/io/micronaut/core/convert/format/Format.html) annotation:
