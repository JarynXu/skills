...
> Upstream path: `src/main/docs/guide/config/propertySource.adoc`  
> Upstream Git blob: `0179eefadf4db988067357e55fa1c7156a746611`  
...
The [PropertySource.of]({api}/io/micronaut/context/env/PropertySource.html) method can be used to create a `PropertySource` from a map of values.

Alternatively one can register a [PropertySourceLoader]({api}/io/micronaut/context/env/PropertySourceLoader.html) by creating a `META-INF/services/io.micronaut.context.env.PropertySourceLoader` file containing a reference to the class name of the `PropertySourceLoader`.
