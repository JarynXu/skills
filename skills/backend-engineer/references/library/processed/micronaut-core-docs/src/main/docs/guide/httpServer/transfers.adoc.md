...
> Upstream path: `src/main/docs/guide/httpServer/transfers.adoc`  
> Upstream Git blob: `7fda7b732b92e78aa9cbaf4df61f3fdd775ee589`  
...
It is possible to return a [File]({jdkapi}/java.base/java/io/File.html) object from your controller method, and the data will be returned to the client. The `Content-Type` header of file responses is calculated based on the name of the file.
...
WARNING: Avoid using [PipedInputStream]({jdkapi}/java.base/java/io/PipedInputStream.html) / [PipedOutputStream]({jdkapi}/java.base/java/io/PipedOutputStream.html) as the backing source for `StreamedFile` or other `InputStream`-based HTTP responses. `PipedInputStream` tracks the liveness of the reader and writer threads and can report a broken pipe when the response is consumed on short-lived threads. This is especially relevant when the `blocking` executor uses virtual threads. Prefer a stable stream source such as a file, URL, or a reactive `Publisher` response. If you must use piped streams, make sure both ends stay bound to long-lived platform threads for the lifetime of the transfer.
