...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/features/external-config.adoc`  
> Upstream Git blob: `1f3ab5cec2d83cb52e7fd6288ee7a80014563eff`  
...
Property values can be injected directly into your beans by using the javadoc:org.springframework.beans.factory.annotation.Value[format=annotation] annotation, accessed through Spring's javadoc:org.springframework.core.env.Environment[] abstraction, or be [bound to structured objects](features/external-config.adoc#features.external-config.typesafe-configuration-properties) through javadoc:org.springframework.boot.context.properties.ConfigurationProperties[format=annotation].
...
. `properties` attribute on your tests.
  Available on javadoc:org.springframework.boot.test.context.SpringBootTest[format=annotation] and the [test annotations for testing a particular slice of your application](testing/spring-boot-applications.adoc#testing.spring-boot-applications.autoconfigured-tests).
...
. [Devtools global settings properties](using/devtools.adoc#using.devtools.globalsettings) in the `$HOME/.config/spring-boot` directory when devtools is active.

Config data files are considered in the following order:

. [Application properties](features/external-config.adoc#features.external-config.files) packaged inside your jar (`application.properties` and YAML variants).
. [Profile-specific application properties](features/external-config.adoc#features.external-config.files.profile-specific) packaged inside your jar (`application-\{profile}.properties` and YAML variants).
. [Application properties](features/external-config.adoc#features.external-config.files) outside of your packaged jar (`application.properties` and YAML variants).
. [Profile-specific application properties](features/external-config.adoc#features.external-config.files.profile-specific) outside of your packaged jar (`application-\{profile}.properties` and YAML variants).
...
See [features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables](features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables) for details.
...
See the [Production ready features](actuator/endpoints.adoc) section for details.
...
TIP: Use the prefix `optional:` if the [locations are optional](features/external-config.adoc#features.external-config.files.optional-prefix) and you do not mind if they do not exist.
...
NOTE: Both directory and file location values are also expanded to check for [profile-specific files](features/external-config.adoc#features.external-config.files.profile-specific).
...
See the example in the [features/external-config.adoc#features.external-config.files.profile-specific](features/external-config.adoc#features.external-config.files.profile-specific) section for more details.
...
You can use this prefix with the `spring.config.location` and `spring.config.additional-location` properties, as well as with [`spring.config.import`](features/external-config.adoc#features.external-config.files.importing) declarations.
...
[NOTE]
====
The last-wins strategy applies at the [location group](features/external-config.adoc#features.external-config.files.location-groups) level.
...
If you have already directly [imported](features/external-config.adoc#features.external-config.files.importing) a profile specific property files then it will not be imported a second time.
...
NOTE: When appropriate, [Profile-specific variants](features/external-config.adoc#features.external-config.files.profile-specific) are also considered for import.
...
By default you can import Java Properties, YAML and [configuration trees](features/external-config.adoc#features.external-config.files.configtree).
...
The configprop:spring.config.import[] configuration property supports file attributes, as shown when specifying the [encoding](features/external-config.adoc#features.external-config.files.importing) or the [extension](features/external-config.adoc#features.external-config.files.importing-extensionless).
...
You can either use environment variables for such purpose, or you can use [configuration trees](reference:features/external-config.adoc#features.external-config.files.configtree).
...
TIP: This feature also supports [specifying the extension](reference:features/external-config.adoc#features.external-config.files.importing-extensionless).
...
For the first case, you can import the YAML or Properties file directly using `spring.config.import` as described [above](features/external-config.adoc#features.external-config.files.importing).
...
This will allow Spring Boot to use the same logic as it does when [relaxed binding](features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding) javadoc:org.springframework.boot.context.properties.ConfigurationProperties[format=annotation].
...
See the [how-to:properties-and-configuration.adoc#howto.properties-and-configuration.short-command-line-arguments](how-to:properties-and-configuration.adoc#howto.properties-and-configuration.short-command-line-arguments) section in "`How-to Guides`" for details.
...
See the [next section](features/external-config.adoc#features.external-config.files.activation-properties) for details.
...
See [how-to:application.adoc#howto.application.customize-the-environment-or-application-context](how-to:application.adoc#howto.application.customize-the-environment-or-application-context) for details.
...
For more details see the [features/external-config.adoc#features.external-config.typesafe-configuration-properties](features/external-config.adoc#features.external-config.typesafe-configuration-properties) section below.
...
TIP: See also the [differences between javadoc:org.springframework.beans.factory.annotation.Value[format=annotation](features/external-config.adoc#features.external-config.typesafe-configuration-properties.vs-value-annotation) and type-safe configuration properties].
...
See the [appendix](specification:configuration-metadata/index.adoc) for details.
...
| Upper case format with underscore as the delimiter (see [features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables](features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables)).
| Numeric values surrounded by underscores (see [features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables](features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding.environment-variables))
...
| [Relaxed binding](features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding)
| Yes
| Limited (see [note below](features/external-config.adoc#features.external-config.typesafe-configuration-properties.vs-value-annotation.note))

| [Meta-data support](specification:configuration-metadata/index.adoc)
| Yes
| No
...
`SpEL` expressions from  [application property files](features/external-config.adoc#features.external-config.files) are not processed at time of parsing these files and populating the environment.
