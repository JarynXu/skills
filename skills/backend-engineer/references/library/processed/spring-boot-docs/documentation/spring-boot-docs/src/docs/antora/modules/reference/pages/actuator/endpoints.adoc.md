...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/actuator/endpoints.adoc`  
> Upstream Git blob: `6943f9ac0a385d92e304060e171e4945d1d34c65`  
...
You can [control access](actuator/endpoints.adoc#actuator.endpoints.controlling-access) to each individual endpoint and [expose them (make them remotely accessible) over HTTP or JMX](actuator/endpoints.adoc#actuator.endpoints.exposing).
...
TIP: To learn more about the Actuator's endpoints and their request and response formats, see the [API documentation](api:rest/actuator/index.adoc).
...
Subject to [sanitization](actuator/endpoints.adoc#actuator.endpoints.sanitization).
...
| `startup`
| Shows the [startup steps data](features/spring-application.adoc#features.spring-application.startup-tracking) collected by the javadoc:org.springframework.core.metrics.ApplicationStartup[].
...
If you want to change only the technologies over which an endpoint is exposed, use the [`include` and `exclude` properties](actuator/endpoints.adoc#actuator.endpoints.exposing) instead.
...
NOTE: If your application is exposed publicly, we strongly recommend that you also [secure your endpoints](actuator/endpoints.adoc#actuator.endpoints.security).
...
See the [API documentation](api:rest/actuator/index.adoc) for details.
...
- No custom [`SanitizingFunction`](how-to:actuator.adoc#howto.actuator.customizing-sanitization) beans apply
...
NOTE: This example assumes that no [`SanitizingFunction`](how-to:actuator.adoc#howto.actuator.customizing-sanitization) beans have been defined.
...
By default, Spring Boot manages your [Application Availability](features/spring-application.adoc#features.spring-application.application-availability) state.
If deployed in a Kubernetes environment, actuator gathers the "`Liveness`" and "`Readiness`" information from the javadoc:org.springframework.boot.availability.ApplicationAvailability[] interface and uses that information in dedicated [health indicators](actuator/endpoints.adoc#actuator.endpoints.health.auto-configured-health-indicators): javadoc:org.springframework.boot.health.application.LivenessStateHealthIndicator[] and javadoc:org.springframework.boot.health.application.ReadinessStateHealthIndicator[].
These indicators are shown on the global health endpoint (`"/actuator/health"`).
They are also exposed as separate HTTP Probes by using [health groups](actuator/endpoints.adoc#actuator.endpoints.health.groups): `"/actuator/health/liveness"` and `"/actuator/health/readiness"`.
...
See the section that describes [how probes behave during the application lifecycle](actuator/endpoints.adoc#actuator.endpoints.kubernetes-probes.lifecycle).
...
Paths can be customized using the `additional-path` property on each group, see [health groups](actuator/endpoints.adoc#actuator.endpoints.health.groups) for details.
...
This means that all the [health groups features](actuator/endpoints.adoc#actuator.endpoints.health.groups) are available for them.
...
If the [liveness state of an application](features/spring-application.adoc#features.spring-application.application-availability.liveness) is broken, Kubernetes tries to solve that problem by restarting the application instance.
...
If the [readiness state of an application instance](features/spring-application.adoc#features.spring-application.application-availability.readiness) is unready, Kubernetes does not route traffic to that instance.
...
Spring Boot publishes [application events during startup and shutdown](features/spring-application.adoc#features.spring-application.application-events-and-listeners),
...
|If enabled, [graceful shutdown processes in-flight requests](web/graceful-shutdown.adoc).
...
TIP: See [how-to:deployment/cloud.adoc#howto.deployment.cloud.kubernetes.container-lifecycle](how-to:deployment/cloud.adoc#howto.deployment.cloud.kubernetes.container-lifecycle) for more information about Kubernetes deployment.
...
| An [SSL Bundle](features/ssl.adoc#features.ssl.bundles) configured.
...
With no prerequisites to indicate that they should be enabled, the `env`, `java`, `os`, and `process` contributors are disabled by default. The `ssl` contributor has a prerequisite of having an [SSL Bundle](features/ssl.adoc#features.ssl.bundles) configured but it is disabled by default.
...
[TIP]
====
Rather than hardcoding those values, you could also [expand info properties at build time](how-to:properties-and-configuration.adoc#howto.properties-and-configuration.expand-properties).
...
See [how-to:build.adoc#howto.build.generate-git-info](how-to:build.adoc#howto.build.generate-git-info) for more detail.
...
See [how-to:build.adoc#howto.build.generate-info](how-to:build.adoc#howto.build.generate-info) for more details.
...
The `info` endpoint publishes information about your SSL certificates (that are configured through [SSL Bundles](features/ssl.adoc#features.ssl.bundles)), see javadoc:org.springframework.boot.info.SslInfo[] for more details.
...
TIP: To automatically generate a CycloneDX SBOM at project build time, please see the [how-to:build.adoc#howto.build.generate-cyclonedx-sbom](how-to:build.adoc#howto.build.generate-cyclonedx-sbom) section.
