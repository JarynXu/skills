...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/features/logging.adoc`  
> Upstream Git blob: `64676c943345734fa7e59e9aab32e58d7661dad3`  
...
TIP: For more details about correlation IDs, please [see this documentation](reference:actuator/tracing.adoc#actuator.micrometer-tracing.logging).
...
Since relaxed binding [always converts environment variables to lowercase](features/external-config.adoc#features.external-config.typesafe-configuration-properties.relaxed-binding.maps-from-environment-variables), it is not possible to configure logging for an individual class in this way.
If you need to configure logging for a class, you can use [the `SPRING_APPLICATION_JSON`](features/external-config.adoc#features.external-config.application-json) variable.
...
[TIP]
====
If you want to use a placeholder in a logging property, you should use [Spring Boot's syntax](features/external-config.adoc#features.external-config.files.property-placeholders) and not the syntax of the underlying framework.
...
* [Elastic Common Schema (ECS)](#features.logging.structured.ecs)
* [Graylog Extended Log Format (GELF)](#features.logging.structured.gelf)
* [Logstash](#features.logging.structured.logstash)
...
If you are using [Custom Log Configuration](#features.logging.custom-log-configuration), update your configuration to respect `CONSOLE_LOG_STRUCTURED_FORMAT` and `FILE_LOG_STRUCTURED_FORMAT` system properties.
