...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/features/spring-application.adoc`  
> Upstream Git blob: `52c49407eb8a1e9285e6530cfcb6ffbecb5a02a0`  
...
If you need a log level other than `INFO`, you can set it, as described in [features/logging.adoc#features.logging.log-levels](features/logging.adoc#features.logging.log-levels).
...
NOTE: Spring Boot provides numerous javadoc:org.springframework.boot.diagnostics.FailureAnalyzer[] implementations, and you can [add your own](how-to:application.adoc#howto.application.failure-analyzer).
...
To do so, you need to [enable the `debug` property](features/external-config.adoc) or [enable `DEBUG` logging](features/logging.adoc#features.logging.log-levels) for javadoc:org.springframework.boot.autoconfigure.logging.ConditionEvaluationReportLoggingListener[].
...
See [features/external-config.adoc](features/external-config.adoc) for details.
...
NOTE: In general, the "Liveness" state should not be based on external checks, such as [health checks](actuator/endpoints.adoc#actuator.endpoints.health).
...
An application is considered live as soon as the context has been refreshed, see [Spring Boot application lifecycle and related Application Events](features/spring-application.adoc#features.spring-application.application-events-and-listeners).
...
An application is considered ready as soon as application and command-line runners have been called, see [Spring Boot application lifecycle and related Application Events](features/spring-application.adoc#features.spring-application.application-events-and-listeners).
...
Spring Boot provides [Kubernetes HTTP probes for "Liveness" and "Readiness" with Actuator Health Endpoints](actuator/endpoints.adoc#actuator.endpoints.kubernetes-probes).
You can get more guidance about [deploying Spring Boot applications on Kubernetes in the dedicated section](how-to:deployment/cloud.adoc#howto.deployment.cloud.kubernetes).
...
. An javadoc:org.springframework.boot.context.event.ApplicationReadyEvent[] is sent after any [application and command-line runners](features/spring-application.adoc#features.spring-application.command-line-runner) have been called.
...
Consider using [application and command-line runners](features/spring-application.adoc#features.spring-application.command-line-runner) instead.
...
Spring Boot can also be configured to expose a [`startup` endpoint](api:rest/actuator/startup.adoc) that provides this information as a JSON document.
