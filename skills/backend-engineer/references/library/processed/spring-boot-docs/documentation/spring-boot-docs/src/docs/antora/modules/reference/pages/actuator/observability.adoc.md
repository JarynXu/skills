...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/actuator/observability.adoc`  
> Upstream Git blob: `6e2efcb04110b7c333bd25d7a2076366a8c34f38`  
...
NOTE: When you annotate methods or classes which are already instrumented (for example, [Spring Data repositories](reference:actuator/metrics.adoc#actuator.metrics.supported.spring-data-repository) or [Spring MVC controllers](reference:actuator/metrics.adoc#actuator.metrics.supported.spring-mvc)), you will get duplicate observations.
In that case you can either disable the automatic instrumentation using [properties](reference:actuator/observability.adoc#actuator.observability.preventing-observations) or an javadoc:io.micrometer.observation.ObservationPredicate[] and rely on your annotations, or you can remove your annotations.
...
Exporting OpenTelemetry traces is only auto-configured when used together with [Micrometer Tracing](actuator/tracing.adoc).
...
This can be done with 3rd-party log bridges, as described in the [Logging with OpenTelemetry](reference:actuator/loggers.adoc#actuator.loggers.opentelemetry) section.
...
However, Micrometer metrics can be exported via OTLP to any OpenTelemetry capable backend using the javadoc:io.micrometer.registry.otlp.OtlpMeterRegistry[], as described in the [Metrics with OTLP](reference:actuator/metrics.adoc#actuator.metrics.export.otlp) section.
