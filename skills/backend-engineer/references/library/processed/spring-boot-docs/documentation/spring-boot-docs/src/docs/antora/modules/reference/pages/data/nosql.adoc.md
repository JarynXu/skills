...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/data/nosql.adoc`  
> Upstream Git blob: `25df9e9104ccddce3b9d9dd68439ab8446281f53`  
...
Custom SSL trust material can be configured in an [SSL bundle](features/ssl.adoc) and applied to the javadoc:org.springframework.data.redis.connection.RedisConnectionFactory[] as shown in this example:
...
Custom SSL trust material can be configured in an [SSL bundle](features/ssl.adoc) and applied to the javadoc:{url-mongodb-driver-sync-javadoc}/com.mongodb.client.MongoClient[] as shown in this example:
...
In the presence of `spring-data-elasticsearch` and Reactor, Spring Boot can also auto-configure a [`ReactiveElasticsearchClient`](data/nosql.adoc#data.nosql.elasticsearch.connecting-using-rest.reactiveclient) and a javadoc:org.springframework.data.elasticsearch.client.elc.ReactiveElasticsearchTemplate[] as beans.
...
Custom SSL trust material can be configured in an [SSL bundle](features/ssl.adoc) and applied to the `CqlSession` as shown in this example:
...
For instance, the following configuration changes the timeout to open a new javadoc:com.couchbase.client.java.Bucket[] and enables SSL support with a reference to a configured [SSL bundle](features/ssl.adoc):
...
To enable SSL, configure the [SSL bundle](features/ssl.adoc) to use by setting the configprop:spring.ldap.embedded.ssl.bundle[] property, as shown in the following example:
