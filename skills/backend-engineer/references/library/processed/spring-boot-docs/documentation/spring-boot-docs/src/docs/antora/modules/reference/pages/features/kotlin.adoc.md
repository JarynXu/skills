...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/features/kotlin.adoc`  
> Upstream Git blob: `d7215bff67616a1c3e4d77283bb3baa974d986a2`  
...
TIP: To generate [your own metadata](specification:configuration-metadata/annotation-processor.adoc) using the annotation processor, {url-kotlin-docs}/kapt.html[`kapt` should be configured] with the `spring-boot-configuration-processor` dependency.
...
If you need the `MockK` equivalent of the Mockito specific [`@MockitoBean` and javadoc:org.springframework.test.context.bean.override.mockito.MockitoSpyBean[format=annotation](testing/spring-boot-applications.adoc#testing.spring-boot-applications.mocking-beans) annotations], you can use https://github.com/Ninja-Squad/springmockk[SpringMockK] which provides similar `@MockkBean` and `@SpykBean` annotations.
