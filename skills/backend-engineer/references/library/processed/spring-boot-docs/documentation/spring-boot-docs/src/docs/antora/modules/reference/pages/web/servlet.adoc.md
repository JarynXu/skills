...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/web/servlet.adoc`  
> Upstream Git blob: `8c98c194f8cb34cb424936ffac96b273fbbef0f3`  
...
* Support for serving static resources, including support for WebJars (covered [later in this document](web/servlet.adoc#web.servlet.spring-mvc.static-content)).
...
* Support for javadoc:org.springframework.http.converter.HttpMessageConverters[] (covered [later in this document](web/servlet.adoc#web.servlet.spring-mvc.message-converters)).
* Automatic registration of javadoc:org.springframework.validation.MessageCodesResolver[] (covered [later in this document](web/servlet.adoc#web.servlet.spring-mvc.message-codes)).
* Static `index.html` support.
* Automatic use of a javadoc:org.springframework.web.bind.support.ConfigurableWebBindingInitializer[] bean (covered [later in this document](web/servlet.adoc#web.servlet.spring-mvc.binding-initializer)).
...
Note that, by default, the [serving of static content](web/servlet.adoc#web.servlet.spring-mvc.static-content) is mapped to `+/**+` and will, therefore, provide a handler for all requests.
...
There are several [known limitations](web/servlet.adoc#web.servlet.embedded-container.jsp-limitations) when using them with embedded servlet containers.
...
See the [Web Properties](appendix:application-properties/index.adoc#appendix.application-properties.web) section of the Appendix.
...
In some cases, errors handled at the controller level are not recorded by web observations or the [metrics infrastructure](actuator/metrics.adoc#actuator.metrics.supported.spring-mvc).
...
See [io/rest-client.adoc#io.rest-client.apiversioning](io/rest-client.adoc#io.rest-client.apiversioning) for details.
...
For example, it cannot scan for endpoints in a package found in a [fully executable jar file](how-to:deployment/installing.adoc) or in `WEB-INF/classes` when running an executable war file.
...
TIP: To see the order of every javadoc:jakarta.servlet.Filter[] in your application, enable debug level logging for the `web` [logging group](features/logging.adoc#features.logging.log-groups) (`logging.level.web=debug`).
...
* [SSL](how-to:webserver.adoc#howto.webserver.configure-ssl)
* [HTTP compression](how-to:webserver.adoc#howto.webserver.enable-response-compression)

Spring Boot tries as much as possible to expose common settings, but this is not always possible.
For those cases, dedicated namespaces offer server-specific customizations (see `server.tomcat`).
For instance, [access logs](how-to:webserver.adoc#howto.webserver.configure-access-logs) can be configured with specific features of the embedded servlet container.
...
* Creating a custom `error.jsp` page does not override the default view for [error handling](web/servlet.adoc#web.servlet.spring-mvc.error-handling).
  [Custom error pages](web/servlet.adoc#web.servlet.spring-mvc.error-handling.error-pages) should be used instead.
