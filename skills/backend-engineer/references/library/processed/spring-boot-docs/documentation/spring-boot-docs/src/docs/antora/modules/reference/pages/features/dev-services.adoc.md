...
> Upstream path: `documentation/spring-boot-docs/src/docs/antora/modules/reference/pages/features/dev-services.adoc`  
> Upstream Git blob: `496364b53a4e3bf468b1bba18fdeaf30cc0e83dc`  
...
When using the Gradle plugin, [configure the task's classpath to include the `developmentOnly` configuration](gradle-plugin:packaging.adoc#packaging-executable.configuring.including-development-only-dependencies).
...
These labels mirror the properties available for [SSL bundles](reference:features/ssl.adoc#features.ssl.jks).
...
These labels mirror the properties available for [SSL bundles](reference:features/ssl.adoc#features.ssl.pem).
...
As well as [using Testcontainers for integration testing](testing/testcontainers.adoc#testing.testcontainers), it's also possible to use them at development time.
...
See [the service connections](testing/testcontainers.adoc#testing.testcontainers.service-connections) section for details of the supported technologies.
...
TIP: If you don't intend to use the [service connections feature](testing/testcontainers.adoc#testing.testcontainers.service-connections) but want to use [`@DynamicPropertySource`](testing/testcontainers.adoc#testing.testcontainers.dynamic-properties) instead, remove the javadoc:org.springframework.boot.testcontainers.service.connection.ServiceConnection[format=annotation] annotation from the javadoc:org.testcontainers.containers.Container[] fields.
