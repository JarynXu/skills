...
> Source: `python/peps@b4f8bb538cffe7bd1401d0f529e721caf4125482`  
> Upstream path: `peps/pep-0484.rst`  
> Upstream Git blob: `df17858d269db1f7c93d3d64dea6788a4acbdb09`  
> Transform: `rst-to-html-to-markdown:docutils+markdownify`  
> This Markdown is generated for agent use. Consult `originals/` when exact upstream bytes matter.

PEP: 484
Title: Type Hints
Author: Guido van Rossum <[guido@python.org](mailto:guido@python.org)>, Jukka Lehtosalo <[jukka.lehtosalo@iki.fi](mailto:jukka.lehtosalo@iki.fi)>, Łukasz Langa <[lukasz@python.org](mailto:lukasz@python.org)>
BDFL-Delegate: Mark Shannon
Discussions-To: [python-dev@python.org](mailto:python-dev@python.org)
Status: Final
...
[PEP 3107](https://peps.python.org/pep-3107) introduced syntax for function annotations, but the semantics
...
simply enables better coordination, as [PEP 333](https://peps.python.org/pep-0333) did for web frameworks.
...
\_\_annotations\_\_ attribute, *no type checking happens at runtime*.
...
The proposal is strongly inspired by [mypy](http://mypy-lang.org). For example, the
...
works at runtime by implementing \_\_getitem\_\_() in the metaclass
...
Gradual typing and the full type system are explained in [PEP 483](https://peps.python.org/pep-0483).

Other approaches from which we have borrowed or to which ours can be
compared and contrasted are described in [PEP 482](https://peps.python.org/pep-0482).

## Rationale and Goals

[PEP 3107](https://peps.python.org/pep-3107) added support for arbitrary annotations on parts of a
...
hinting](https://www.artima.com/weblogs/viewpost.jsp?thread=85551), which is listed as the first possible use case
in said PEP.
...
runtime type checking -- in particular the get\_type\_hints()
...
with the @no\_type\_check decorator should be treated as having
no annotations.
...
(Note that the return type of \_\_init\_\_ ought to be annotated with
-> None. The reason for this is subtle. If \_\_init\_\_ assumed
a return annotation of -> None, would that mean that an
argument-less, un-annotated \_\_init\_\_ method should still be
type-checked? Rather than leaving this ambiguous or introducing an
exception to the exception, we simply say that \_\_init\_\_ ought to
...
The syntax leverages [PEP 3107](https://peps.python.org/pep-3107)-style annotations with a number of
...
The Generic base class uses a metaclass that defines \_\_getitem\_\_
...
(\_\_init\_\_ or \_\_new\_\_) uses T in its signature, and a
...
inferred type to be \_exactly\_ one of the constraint types, while an
...
introduction to these concepts can be found on [Wikipedia](https://en.wikipedia.org/wiki/Covariance_and_contravariance_%28computer_science%29) and in [PEP 483](https://peps.python.org/pep-0483); here we just show how to control
...
[PEP 3141](https://peps.python.org/pep-3141) defines Python's numeric tower, and the stdlib module
...
it is constrained by the rules from the section [Acceptable type hints](#acceptable-type-hints)
above.
...
Without Type[] the best we could do to annotate new\_user()
would be:
...
Now when we call new\_user() with a specific subclass of User a
...
above example calling e.g. new\_user(Union[BasicUser, ProUser]) is
...
new\_user() calls user\_class() this implies that all subclasses
...
\_\_repr\_\_() and \_\_mro\_\_). Such a variable can be called with
...
TYPE\_CHECKING, that is considered True during type checking
...
"forward reference", to hide the expensive\_mod reference from the
...
\_\_ are assumed to be positional-only, except if their names also
end with \_\_:
...
the generic type Generator[yield\_type, send\_type, return\_type] provided by typing.py module:
...
Coroutines introduced in [PEP 492](https://peps.python.org/pep-0492) are annotated with the same syntax as
...
correspond to those of Generator, namely Coroutine[T\_co, T\_contra, V\_co],
for example:
...
storing annotations in the \_\_annotations\_\_ attribute of the function
...
- a @no\_type\_check decorator on a class or function;
...
discussion under [Rejected alternatives](#rejected-alternatives) below.
...
without giving it an initial value. This can be done using [PEP 526](https://peps.python.org/pep-0526)
...
was added in Python 3.6 through [PEP 526](https://peps.python.org/pep-0526).)
...
- Modules and variables imported into the stub are not considered
  exported from the stub unless the import uses the import ... as ... form or the equivalent from ... import ... as ... form.
...
- Just like in [normal Python files](https://docs.python.org/3/reference/import.html#submodules), submodules
  ...
  where \_\_init\_\_.pyi contains a line such as from . import ham
...
the \_\_getitem\_\_() method of the bytes type can be described as
follows:
...
functools.singledispatch(). (See [PEP 443](https://peps.python.org/pep-0443), especially its section
...
Some functions, such as map or bytes.\_\_getitem\_\_ above, can't
...
hints, [PEP 561](https://peps.python.org/pep-0561). It supports separately installable packages containing
...
There is a [shared repository](https://github.com/python/typeshed) where useful stubs are being
...
- Dict, used as Dict[key\_type, value\_type]
- DefaultDict, used as DefaultDict[key\_type, value\_type],
  a generic variant of collections.defaultdict
- List, used as List[element\_type]
- Set, used as Set[element\_type]. See remark for AbstractSet
  below.
- FrozenSet, used as FrozenSet[element\_type]
...
- ContextManager
- Coroutine
- Generator, used as Generator[yield\_type, send\_type, return\_type]. This represents the return value of generator
...
- Reversible, to test for \_\_reversed\_\_
- SupportsAbs, to test for \_\_abs\_\_
- SupportsComplex, to test for \_\_complex\_\_
- SupportsFloat, to test for \_\_float\_\_
- SupportsInt, to test for \_\_int\_\_
- SupportsRound, to test for \_\_round\_\_
- SupportsBytes, to test for \_\_bytes\_\_
...
- NamedTuple, used as
  NamedTuple(type\_name, [(field\_name, field\_type), ...])
  and equivalent to
  collections.namedtuple(type\_name, [field\_name, ...]).
...
- @no\_type\_check, a decorator to disable type checking per class or
  function (see below)
- @no\_type\_check\_decorator, a decorator to create your own decorators
  with the same meaning as @no\_type\_check (see below)
- @type\_check\_only, a decorator only available during type checking
  for use in stub files (see above); marks a class or function as
  unavailable during runtime
- @overload, described earlier
- get\_type\_hints(), a utility function to retrieve the type hints from a
  function or method. Given a function or method object, it returns
  a dict with the same format as \_\_annotations\_\_, but evaluating
  forward references (which are given as string literals) as expressions
  in the context of the original function or method definition.
- TYPE\_CHECKING, False at runtime but True to type checkers
...
They can be implemented by defining the \_\_getitem\_\_() method on
...
One line of argument points out that [PEP 3107](https://peps.python.org/pep-3107) explicitly supports
...
[PEP 411](https://peps.python.org/pep-0411)) until Python 3.6 is released. The fastest conceivable scheme
...
the updated schedule see [PEP 563](https://peps.python.org/pep-0563).)
...
proposal defines a decorator @no\_type\_check which disables the
default interpretation of annotations as type hints in a given class
or function. It also defines a meta-decorator
@no\_type\_check\_decorator which can be used to decorate a decorator
...
The problem with type hints is that annotations (per [PEP 3107](https://peps.python.org/pep-3107), and
...
A compromise is possible where a \_\_future\_\_ import could enable
...
Such a \_\_future\_\_ import statement may be proposed in a separate
PEP.

(*UPDATE:* That \_\_future\_\_ import statement and its consequences
are discussed in [PEP 563](https://peps.python.org/pep-0563).)
...
- It's ugly. The single colon in Python has many uses, and all of
  ...
  ([the idea](https://www.artima.com/weblogs/viewpost.jsp?thread=85551)
  long predates [PEP 3107](https://peps.python.org/pep-3107)). It is also used in the same
...
introduction](https://aroberge.blogspot.com/2015/01/type-hinting-in-python-focus-on.html) of a where keyword, and Cobra-inspired
...
would apply to a new \_\_future\_\_ import.
...
- A decorator, e.g. @typehints(name=str, returns=str). This could
  ...
  must be repeated), and a far cry in elegance from the [PEP 3107](https://peps.python.org/pep-3107)
  notation.
...
A live draft for this PEP lives on [GitHub](https://github.com/python/typing). There is also an
[issue tracker](https://github.com/python/typing/issues), where much of the technical discussion takes
place.

The draft on GitHub is updated regularly in small increments. The
[official PEPS repo](https://hg.python.org/peps/file/tip/pep-0484.txt) is (usually) only updated when a new draft
...
mentioned in [PEP 482](https://peps.python.org/pep-0482). Many thanks to their creators, in alphabetical
