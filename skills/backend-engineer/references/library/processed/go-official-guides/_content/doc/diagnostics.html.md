...
> Upstream path: `_content/doc/diagnostics.html`  
> Upstream Git blob: `f7717c1b7af6575d73feefaf76134b31c8a684f9`  
...
of code. The Go runtime provides [profiling data](/pkg/runtime/pprof/) in the format expected by the
[pprof visualization tool](https://github.com/google/pprof/blob/master/doc/README.md).
The profiling data can be collected during testing
via `go` `test` or endpoints made available from the [net/http/pprof](/pkg/net/http/pprof/) package. Users need to collect the profiling data and use pprof tools to filter
...
On Linux, [perf tools](https://perf.wiki.kernel.org/index.php/Tutorial)
...
The Go tools provide text, graph, and [callgrind](http://valgrind.org/docs/manual/cl-manual.html)
visualization of the profile data using
`go tool pprof`.
Read [Profiling Go programs](/blog/profiling-go-programs)
...
Another way to visualize profile data is a [flame graph](http://www.brendangregg.com/flamegraphs.html).
Flame graphs allow you to move in a specific ancestry path, so you can zoom
in/out of specific sections of code.
The [upstream pprof](https://github.com/google/pprof)
...
their custom profiles via [pprof.Profile](/pkg/runtime/pprof/#Profile)
...
handler on :7777 at /custom\_debug\_path/profile:
...
flag `-dwarflocationlists`. The flag causes the compiler to
...
[core dump debugging](/wiki/CoreDumpDebugging)
...
- GODEBUG=cpu.*extension*=off disables use of instructions from the
  ...
  *extension* is the lower case name for the instruction set extension
  such as *sse41* or *avx*.
