...
> Upstream path: `docs/userguide/calling.rst`  
> Upstream Git blob: `b014357e2b6970a6a9ac2bdc7ce8596e0407e073`  
...
- T.delay(arg, kwarg=value)
  :   Star arguments shortcut to .apply\_async.
      (.delay(\*args, \*\*kwargs) calls .apply\_async(args, kwargs)).
- T.apply\_async((arg,), {'kwarg': value})
- T.apply\_async(countdown=10)
  :   executes in 10 seconds from now.
- T.apply\_async(eta=now + timedelta(seconds=10))
  :   executes in 10 seconds from now, specified using eta
- T.apply\_async(countdown=60, expires=120)
  :   executes in one minute from now, but expires after 2 minutes.
- T.apply\_async(expires=now + timedelta(days=2))
  :   expires in 2 days, set using :class:`~datetime.datetime`.
- T.apply\_async(task\_id=f'my\_own\_task\_id')
...
Using :meth:`~@Task.apply\_async` instead you have to write:
...
you can use :meth:`~@send\_task` to call the task by name instead.

So delay is clearly convenient, but if you want to set additional execution
options you have to use apply\_async.
...
## [Linking (callbacks/errbacks)](#toc-entry-2)
...
it can be added to the task using the link\_error execution
option:
...
In addition, both the link and link\_error options can be expressed
as a list:
...
exceeds visibility\_timeout (see :ref:`redis-caveats`).
...
consumer\_timeout is 15 minutes.
...
closed with a PRECONDITION\_FAILED channel exception.
...
should specify the consumer\_timeout parameter greater than or equal to
your countdown value. For example, you can specify a very large value
of consumer\_timeout = 31622400000, which is equal to 1 year
in milliseconds, to avoid problems in the future.

## [Expiration](#toc-entry-5)
...
## [Message Sending Retry](#toc-entry-6)
...
## [Connection Error Handling](#toc-entry-7)
...
limits. Enable confirm\_publish in :setting:`broker\_transport\_options`
to detect this.

## [Serializers](#toc-entry-8)
...
so every message in Celery has a content\_type header that
describes the serialization method used to encode it.

The default serializer is JSON, but you can
change this using the :setting:`task\_serializer` setting,
...
:setting:`accept\_content`. By default, only JSON is accepted,
...
> 3. The :setting:`task\_serializer` setting.
...
> 3. The :setting:`task\_compression` attribute.
...
See the :setting:`broker\_pool\_limit` setting for more information.
...
## [Results options](#toc-entry-12)

You can enable or disable result storage using the :setting:`task\_ignore\_result`
setting or by using the ignore\_result option:
...
set the :setting:`result\_extended` setting to True.

Note

result\_extended controls what *Celery* includes as extended task metadata,
...
When you call tasks manually using apply\_async/delay, that periodic task
context is usually not present unless you add it explicitly (e.g. via message
headers/properties in apply\_async options). For example:
