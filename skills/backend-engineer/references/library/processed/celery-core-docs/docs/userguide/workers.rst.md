...
> Upstream path: `docs/userguide/workers.rst`  
> Upstream Git blob: `63c96b051faa3ff10f19c771c6964fd8332dc695`  
> Transform: `rst-to-html-to-markdown:docutils+markdownify`  
> This Markdown is generated for agent use. Consult `originals/` when exact upstream bytes matter.

# Workers Guide

## [Starting the worker](#toc-entry-1)
...
Note for :pypi:`supervisor` users

The % sign must be escaped by adding a second one: %%h.

## [Stopping the worker](#toc-entry-2)
...
be lost (i.e., unless the tasks have the :attr:`~@Task.acks\_late`
option set).
...
If the environment variable REMAP\_SIGTERM is set to SIGQUIT, the worker will also initiate
...
If the warm shutdown already started, the transition to cold shutdown will run a signal handler on\_cold\_shutdown
...
successful one interrupted during cleanup. See [issue #10271](https://github.com/celery/celery/issues/10271) for a full analysis.

Prefer :ref:`worker-warm-shutdown` to allow tasks to complete cleanly. If cold shutdown is
unavoidable, avoid constructs such as Django's on\_commit which rely on guarantees that
...
allow :setting:`worker\_soft\_shutdown\_timeout` seconds for all currently executing tasks to finish before
...
behavior. To enable the soft shutdown, set :setting:`worker\_soft\_shutdown\_timeout` to a positive float value.
The soft shutdown will be skipped if there are no tasks running. To force the soft shutdown, *also* enable the
:setting:`worker\_enable\_soft\_shutdown\_on\_idle` setting.

Warning

If the worker is not running any task but has ETA tasks reserved, the soft shutdown will not be initiated
unless the :setting:`worker\_enable\_soft\_shutdown\_on\_idle` setting is enabled, which may lead to task loss
during the cold shutdown. When using ETA tasks, it is recommended to enable the soft shutdown on idle.
Experiment which :setting:`worker\_soft\_shutdown\_timeout` value works best for your setup to reduce the risk
...
For example, when setting worker\_soft\_shutdown\_timeout=3, the worker will allow 3 seconds for all currently
...
## [Restarting the worker](#toc-entry-3)
...
## [Automatic re-connection on connection loss to broker](#toc-entry-4)

Unless :setting:`broker\_connection\_retry\_on\_startup` is set to False,
Celery will automatically retry reconnecting to the broker after the first
connection loss. :setting:`broker\_connection\_retry` controls whether to automatically
retry reconnecting to the broker for subsequent reconnects.

If :setting:`worker\_cancel\_long\_running\_tasks\_on\_connection\_loss` is set to True,
...
tasks that are currently running multiplied by :setting:`worker\_prefetch\_multiplier`.
...
to :setting:`worker\_enable\_prefetch\_count\_reduction`.

## [Process Signals](#toc-entry-5)
...
## [Variables in file paths](#toc-entry-6)
...
:meth:`~@control.rate\_limit`, and :meth:`~@control.ping`.

Sending the :control:`rate\_limit` command and keyword arguments:
...
command:
:   :program:`celery -A proj control revoke <task\_id>`
...
specified using the CELERY\_WORKER\_REVOKES\_MAX environment
...
CELERY\_WORKER\_REVOKE\_EXPIRES environment variable.

Memory limits can also be set for successful tasks through the
CELERY\_WORKER\_SUCCESSFUL\_MAX and
CELERY\_WORKER\_SUCCESSFUL\_EXPIRES environment variables, and
...
### revoke\_by\_stamped\_headers: Revoking tasks by their stamped headers
...
command:
:   :program:`celery -A proj control revoke\_by\_stamped\_headers <header=value>`
...
The revoke\_by\_stamped\_headers method also accepts a list argument, where it will revoke
...
This will revoke all of the tasks that have a stamped header header\_A with value value\_1,
and all of the tasks that have a stamped header header\_B with values value\_2 or value\_3.
...
Time limits can also be set using the :setting:`task\_time\_limit` /
:setting:`task\_soft\_time\_limit` settings. You can also specify time
...
and hard time limits for a task — named time\_limit.

Example changing the time limit for the tasks.crawl\_the\_web task
...
:setting:`worker\_disable\_rate\_limits` setting enabled.

## [Max tasks per child setting](#toc-entry-12)
...
or using the :setting:`worker\_max\_tasks\_per\_child` setting.

## [Max memory per child setting](#toc-entry-13)
...
or using the :setting:`worker\_max\_memory\_per\_child` setting.

## [Autoscaling](#toc-entry-14)
...
You can specify a custom autoscaler with the :setting:`worker\_autoscaler` setting.

## [Queues](#toc-entry-15)

A worker instance can consume from any number of queues.
By default it will consume from all queues defined in the
:setting:`task\_queues` setting (that if not specified falls back to the
...
If the queue name is defined in :setting:`task\_queues` it will use that
...
:setting:`task\_create\_missing\_queues` option).

You can also tell the worker to start and stop consuming from a queue at
run-time using the remote control commands :control:`add\_consumer` and
:control:`cancel\_consumer`.

### Queues: Adding consumers

The :control:`add\_consumer` control command will tell one or more workers
...
The same can be accomplished dynamically using the :meth:`@control.add\_consumer` method:
...
You can cancel a consumer by queue name using the :control:`cancel\_consumer`
control command.
...
:meth:`@control.cancel\_consumer` method:
...
the :control:`active\_queues` control command:
...
:meth:`~celery.app.control.Inspect.active\_queues` method:
...
## [Inspecting workers](#toc-entry-16)
...
## [Additional Commands](#toc-entry-17)
...
## [Writing your own remote control commands](#toc-entry-18)
