...
> Upstream path: `docs/userguide/optimizing.rst`  
> Upstream Git blob: `00ee54481afb03d455cf33cc922c6282b264a4bc`  
...
The point of this exercise [[\*]](#footnote-1) is to show that there's a limit
...
You can tweak the :setting:`broker\_pool\_limit` setting to minimize
...
or by using :setting:`task\_routes`:
...
The delivery\_mode changes how the messages to this queue are delivered.
...
argument (or use the :setting:`task\_routes` setting):
...
available worker nodes that may be able to process them sooner [[†]](#footnote-2),
or that the messages may not even fit in memory.

The workers' default prefetch count is the
:setting:`worker\_prefetch\_multiplier` setting multiplied by the number
of concurrency slots [[‡]](#footnote-3) (processes/threads/green-threads).
...
prefetching by enabling :setting:`worker\_disable\_prefetch`. With this
...
:setting:`worker\_max\_tasks\_per\_child` and :setting:`worker\_max\_memory\_per\_child`
settings.
...
tasks. For example, if you use a :setting:`worker\_max\_tasks\_per\_child` of 1
...
:setting:`worker\_max\_memory\_per\_child`.
...
This is the concurrency setting; :setting:`worker\_concurrency` or the
