...
> Upstream path: `framework-docs/modules/ROOT/pages/web/webflux/controller/ann-validation.adoc`  
> Upstream Git blob: `67fb1256a889b5371aafc20f1ffffdf79fa9186f`  
...
[.small]#[See equivalent in the Servlet stack](web/webmvc/mvc-controller/ann-validation.adoc)#

Spring WebFlux has built-in [Validation](core/validation/validator.adoc) for
`@RequestMapping` methods, including [Java Bean Validation](core/validation/beanvalidation.adoc).
Validation may be applied at one of two levels:

1. Java Bean Validation is applied individually to an
[@ModelAttribute](web/webflux/controller/ann-methods/modelattrib-method-args.adoc),
[@RequestBody](web/webflux/controller/ann-methods/requestbody.adoc), and
[@RequestPart](web/webflux/controller/ann-methods/multipart-forms.adoc) method parameter
...
[WebFlux config](web/webflux/config.adoc#webflux-config-validation), or locally
through an [@InitBinder](web/webflux/controller/ann-initbinder.adoc) method in an
`@Controller` or `@ControllerAdvice`. You can also use multiple validators.

NOTE: If a controller has a class level `@Validated`, then
[method validation is applied](core/validation/beanvalidation.adoc#validation-beanvalidation-spring-method)
...
The [Error Responses](web/webflux/ann-rest-exceptions.adoc) section provides further
