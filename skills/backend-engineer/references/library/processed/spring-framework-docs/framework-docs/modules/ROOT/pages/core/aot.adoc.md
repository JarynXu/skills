...
> Upstream path: `framework-docs/modules/ROOT/pages/core/aot.adoc`  
> Upstream Git blob: `1fbaed3a503ea418eeae7864020f02cf1b51f52d`  
...
For AOT support specific to integration tests, see [Ahead of Time Support for Tests](testing/testcontext-framework/aot.adoc).
...
Refresh for AOT processing only applies a subset of what happens with a [regular `refresh`](core/beans/introduction.adoc).
AOT processing can be triggered as follows:

include-code::./AotProcessingSample[tag=aotcontext]

In this mode, [`BeanFactoryPostProcessor` implementations](core/beans/factory-extension.adoc#beans-factory-extension-factory-postprocessors) are invoked as usual.
...
For instance [`AutowiredAnnotationBeanPostProcessor`](core/beans/factory-extension.adoc#beans-factory-extension-bpp-examples-aabpp) implements this interface to generate code that injects members annotated with `@Autowired`.
...
[this tip](core/beans/classpath-scanning.adoc#beans-factorybeans-annotations-lazy-injection-points)
...
common types (see [Spring Type Conversion](core/validation/convert.adoc)), some
...
```txt,indent=0,subs="verbatim,quotes"
