...
> Upstream path: `framework-docs/modules/ROOT/pages/testing/testcontext-framework/aot.adoc`  
> Upstream Git blob: `df049c3eb6e277abca27248bb81ebf2cb856b8e5`  
...
The testing support extends Spring's [core AOT support](core/aot.adoc) with the
following features.
...
* Build-time AOT processing: each unique test `ApplicationContext` in the current project
  will be [refreshed for AOT processing](core/aot.adoc#aot.refresh).
* Runtime AOT support: when executing in AOT runtime mode, a Spring integration test will
  ...
  [context cache](testing/testcontext-framework/ctx-management/caching.adoc).
...
* See [Runtime Hints](core/aot.adoc#aot.hints) for details on Spring's core runtime hints
