...
> Upstream path: `framework-docs/modules/ROOT/pages/core/beans/factory-nature.adoc`  
> Upstream Git blob: `c861e4660f8fa86d9d108f40141523ad3d9d97c9`  
...
For details, see [Using `@PostConstruct` and `@PreDestroy`](core/beans/annotation-config/postconstruct-and-predestroy-annotations.adoc).
...
[Container Extension Points](core/beans/factory-extension.adoc).
...
the [`@PostConstruct`](core/beans/annotation-config/postconstruct-and-predestroy-annotations.adoc) annotation or
...
`@Bean`. See [Receiving Lifecycle Callbacks](core/beans/java/bean-annotation.adoc#beans-java-lifecycle-callbacks). Consider the following example:
...
unnecessarily couples the code to Spring. Alternatively, we suggest using
the [`@PreDestroy`](core/beans/annotation-config/postconstruct-and-predestroy-annotations.adoc) annotation or
...
[Receiving Lifecycle Callbacks](core/beans/java/bean-annotation.adoc#beans-java-lifecycle-callbacks). Consider the following definition:
...
* The [`@PostConstruct` and `@PreDestroy` annotations](core/beans/annotation-config/postconstruct-and-predestroy-annotations.adoc)
...
[Additional Capabilities of the `ApplicationContext`](core/beans/context-introduction.adoc).
...
(as described in [Autowiring Collaborators](core/beans/dependencies/factory-autowire.adoc)) can provide a dependency of type
...
[Using `@Autowired`](core/beans/annotation-config/autowired.adoc).
...
| [Instantiating Beans](core/beans/definition.adoc#beans-factory-class)

| `BeanFactoryAware`
| Declaring `BeanFactory`.
| [The `BeanFactory` API](core/beans/beanfactory.adoc)
...
| [Notifications](integration/jmx/notifications.adoc)

| `ResourceLoaderAware`
| Configured loader for low-level access to resources.
| [Resources](web/webflux-webclient/client-builder.adoc#webflux-client-builder-reactor-resources)

| `ServletConfigAware`
| Current `ServletConfig` the container runs in. Valid only in a web-aware Spring
  `ApplicationContext`.
| [Spring MVC](web/webmvc.adoc#mvc)
