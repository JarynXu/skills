...
> Upstream path: `framework-docs/modules/ROOT/pages/web/webmvc/message-converters.adoc`  
> Upstream Git blob: `c9e48ff2c82bca414dfdb640208d6157d867af46`  
...
[.small]#[See equivalent in the Reactive stack](web/webflux/reactive-spring.adoc#webflux-codecs)#
...
Concrete implementations for the main media (MIME) types are provided in the framework and are, by default, registered with the `RestClient` and `RestTemplate` on the client side and with `RequestMappingHandlerAdapter` on the server side (see [Configuring Message Converters](web/webmvc/mvc-config/message-converters.adoc)).
