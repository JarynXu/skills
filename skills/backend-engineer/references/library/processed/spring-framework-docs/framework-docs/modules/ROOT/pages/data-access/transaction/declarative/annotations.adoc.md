...
> Upstream path: `framework-docs/modules/ROOT/pages/data-access/transaction/declarative/annotations.adoc`  
> Upstream Git blob: `aaea324a5ff002c61c72f7e5a54ebcb774b2a5e7`  
...
[Cancel Signals](data-access/transaction/programmatic.adoc#tx-prog-operator-cancel)
...
by default as well. See [Transaction Management](testing/testcontext-framework/tx.adoc)
...
| The default mode (`proxy`) processes annotated beans to be proxied by using Spring's AOP
  ...
  (See [Spring configuration](core/aop/using-aspectj.adoc#aop-aj-ltw-spring) for details
...
| Applies to `proxy` mode only. Controls what type of transactional proxies are created
  ...
  created. (See [Proxying Mechanisms](core/aop/proxying.adoc) for a detailed examination
...
| Defines the order of the transaction advice that is applied to beans annotated with
  ...
  advice, see [Advice Ordering](core/aop/ataspectj/advice.adoc#aop-ataspectj-advice-ordering).)
...
interface-based proxies are created. (See [Proxying Mechanisms](core/aop/proxying.adoc)
...
and not in your services. See [MVC](web/webmvc/mvc-servlet.adoc) for more information.
...
| [propagation](data-access/transaction/declarative/tx-propagation.adoc)
| `enum`: `Propagation`
...
TIP: See
[Rollback rules](data-access/transaction/declarative/rolling-back.adoc#transaction-declarative-rollback-rules)
...
[Spring's meta-annotation support](core/beans/classpath-scanning.adoc#beans-meta-annotations)
