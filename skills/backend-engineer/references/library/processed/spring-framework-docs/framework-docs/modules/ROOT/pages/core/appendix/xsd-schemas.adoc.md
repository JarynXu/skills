...
> Upstream path: `framework-docs/modules/ROOT/pages/core/appendix/xsd-schemas.adoc`  
> Upstream Git blob: `c6532deebb7528731379cae5addd400886df84bd`  
...
Java::
+
```java,indent=0,subs="verbatim,quotes",chomp="-packages"
...
Kotlin::
+
```kotlin,indent=0,subs="verbatim,quotes",chomp="-packages"
	package jakarta.persistence
...
loaded from the supplied [`Resource`](web/webflux-webclient/client-builder.adoc#webflux-client-builder-reactor-resources) location).
...
These tags are comprehensively covered in the chapter entitled [Aspect Oriented Programming with Spring](core/aop.adoc)
.
...
specified properties file (as a [Spring resource location](web/webflux-webclient/client-builder.adoc#webflux-client-builder-reactor-resources)). This element
is a convenience mechanism that sets up a [`PropertySourcesPlaceholderConfigurer`](core/beans/factory-extension.adoc#beans-factory-placeholderconfigurer)
...
* Spring's [`@Configuration`](core/beans/basics.adoc#beans-factory-metadata) model
* [`@Autowired`/`@Inject`](core/beans/annotation-config.adoc), `@Value`, and `@Lookup`
...
* Spring's [`@EventListener`](core/beans/context-introduction.adoc#context-functionality-events-annotation)
...
[`@Transactional`](data-access/transaction/declarative/annotations.adoc) annotation;
you can use the <<data-access.adoc#tx-decl-explained, `<tx:annotation-driven/>`>>
element for that purpose. Similarly, Spring's
[caching annotations](integration/cache/annotations.adoc) need to be explicitly
[enabled](integration/cache/annotations.adoc#cache-annotation-enable) as well.

[[xsd-schemas-context-component-scan]]
### Using `<component-scan/>`

This element is detailed in the section on [annotation-based container configuration](core/beans/annotation-config.adoc).

[[xsd-schemas-context-ltw]]
### Using `<load-time-weaver/>`

This element is detailed in the section on [load-time weaving with AspectJ in the Spring Framework](core/aop/using-aspectj.adoc#aop-aj-ltw).

[[xsd-schemas-context-sc]]
### Using `<spring-configured/>`

This element is detailed in the section on [using AspectJ to dependency inject domain objects with Spring](core/aop/using-aspectj.adoc#aop-atconfigurable).

[[xsd-schemas-context-mbe]]
### Using `<mbean-export/>`

This element is detailed in the section on [configuring annotation-based MBean export](integration/jmx/naming.adoc#jmx-context-mbeanexport).
...
in [dependencies and configuration in detail](core/beans/dependencies/factory-properties-detailed.adoc)
(and, indeed, in that entire [chapter](web/webmvc-view/mvc-xslt.adoc#mvc-view-xslt-beandefs)).
...
in the appendix entitled [XML Schema Authoring](core/appendix/xml-custom.adoc)).
