...
> Upstream path: `framework-docs/modules/ROOT/pages/core/resources.adoc`  
> Upstream Git blob: `cfe100f6d357f8849be49f4434e1a0b86d386bb5`  
...
`constructor` and `byType` autowiring modes (as described in [Autowiring Collaborators](core/beans/dependencies/factory-autowire.adoc))
...
For more information, see [Using `@Autowired`](core/beans/annotation-config/autowired.adoc).
...
[Environment Abstraction](core/beans/environment.adoc)). The template path can then be referenced via the `@Value`
annotation using a property placeholder (see [Using `@Value`](core/beans/annotation-config/value-annotations.adoc)). Spring will
