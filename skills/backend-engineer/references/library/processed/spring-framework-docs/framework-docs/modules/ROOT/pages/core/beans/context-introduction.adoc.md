...
> Upstream path: `framework-docs/modules/ROOT/pages/core/beans/context-introduction.adoc`  
> Upstream Git blob: `affad4a1bec62fea2dea72ba9bf4eb1caebf4e74`  
...
As discussed in the [chapter introduction](core/beans/introduction.adoc), the `org.springframework.beans.factory`
...
See [the `@EventListener` Observability section](integration/observability.adoc#observability.application-events)
...
of the annotation that defines a [`SpEL` expression](core/expressions.adoc), which should match
...
[regular `@Async` support](integration/scheduling.adoc#scheduling-annotation-support-async).
...
[dedicated appendix section](core/appendix/application-startup-steps.adoc).
