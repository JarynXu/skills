...
> Upstream path: `framework-docs/modules/ROOT/pages/web/webmvc/mvc-servlet/exceptionhandlers.adoc`  
> Upstream Git blob: `3a0d285b02e9d248229548fe17f4f15cd71a35ba`  
...
[.small]#[See equivalent in the Reactive stack](web/webflux/dispatcher-handler.adoc#webflux-dispatcher-exceptions)#
...
| Resolves exceptions raised by Spring MVC and maps them to HTTP status codes.
  See also alternative `ResponseEntityExceptionHandler` and [Error Responses](web/webmvc/mvc-ann-rest-exceptions.adoc).
...
| Resolves exceptions by invoking an `@ExceptionHandler` method in a `@Controller` or a
  `@ControllerAdvice` class. See [@ExceptionHandler methods](web/webmvc/mvc-controller/ann-exceptionhandler.adoc).
|===
...
The [MVC Config](web/webmvc/mvc-config.adoc) automatically declares built-in resolvers for default Spring MVC
