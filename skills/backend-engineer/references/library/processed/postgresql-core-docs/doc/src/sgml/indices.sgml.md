...
> Upstream path: `doc/src/sgml/indices.sgml`  
> Upstream Git blob: `55f39b0df2f2e6cedcd07c38548937a5ce4f15cc`  
...
```
SELECT * FROM orders WHERE billed IS NOT TRUE AND order_nr order_nr at all, e.g.:
...
```
SELECT f(x) FROM tab WHERE f(x) f() is an expensive-to-compute function.
