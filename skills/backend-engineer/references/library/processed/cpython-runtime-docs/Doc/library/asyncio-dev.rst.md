...
> Upstream path: `Doc/library/asyncio-dev.rst`  
> Upstream Git blob: `713b40d746680af0e809cc62ec8892e895f8ac3a`  
...
- Many non-threadsafe asyncio APIs (such as :meth:`loop.call\_soon` and
  :meth:`loop.call\_at` methods) raise an exception if they are called
  from a wrong thread.
- The execution time of the I/O selector is logged if it takes too long to
  perform an I/O operation.
- Callbacks taking longer than 100 milliseconds are logged. The
  :attr:`loop.slow\_callback\_duration` attribute can be used to set the
...
:meth:`loop.call\_soon\_threadsafe` method should be used. Example:
...
low-level asyncio API, the :meth:`loop.call\_soon\_threadsafe` method
should be used, e.g.:
...
:func:`run\_coroutine\_threadsafe` function should be used. It returns a
...
The :meth:`loop.run\_in\_executor` method can be used with a
...
aforementioned :meth:`loop.run\_in\_executor` method can also be used
...
event loop. See the :meth:`loop.run\_in\_executor` method for more
details.
...
or the coroutine is not scheduled with :meth:`asyncio.create\_task`, asyncio
...
:meth:`asyncio.create\_task` function:
...
If a :meth:`Future.set\_exception` is called but the Future object is
...
:func:`sys.set\_asyncgen\_hooks` function to register callback functions. These
callbacks update the list of running asynchronous generators to keep it in a
consistent state.

When the :meth:`loop.shutdown\_asyncgens() <asyncio.loop.shutdown\_asyncgens>`
...
:meth:`~agen.\_\_anext\_\_` / :meth:`~agen.athrow` / :meth:`~agen.aclose` call is in
