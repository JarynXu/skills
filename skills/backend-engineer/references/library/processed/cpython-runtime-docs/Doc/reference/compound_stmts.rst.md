...
> Upstream path: `Doc/reference/compound_stmts.rst`  
> Upstream Git blob: `e74f3262ed540cd4a7476f557771c9fb1e4e2194`  
...
The :token:`~python-grammar:starred\_expression\_list` expression is evaluated
...
continues in the surrounding code and on the invocation stack. [[1]](#footnote-1)
...
and :keyword:`else <except\_else>` clauses.
...
1. The context expression (the expression given in the
   :token:`~python-grammar:with\_item`) is evaluated to obtain a context manager.
2. The context manager's :meth:`~object.\_\_enter\_\_` is loaded for later use.
3. The context manager's :meth:`~object.\_\_exit\_\_` is loaded for later use.
4. The context manager's :meth:`~object.\_\_enter\_\_` method is invoked.
5. If a target was included in the :keyword:`with` statement, the return value
   from :meth:`~object.\_\_enter\_\_` is assigned to it.

   Note

   The :keyword:`with` statement guarantees that if the :meth:`~object.\_\_enter\_\_`
   method returns without an error, then :meth:`~object.\_\_exit\_\_` will always be
   called. Thus, if an error occurs during the assignment to the target list,
   it will be treated the same as an error occurring within the suite would
   be. See step 7 below.
6. The suite is executed.
7. The context manager's :meth:`~object.\_\_exit\_\_` method is invoked. If an exception
   caused the suite to be exited, its type, value, and traceback are passed as
   arguments to :meth:`~object.\_\_exit\_\_`. Otherwise, three :const:`None` arguments are
   supplied.

   If the suite was exited due to an exception, and the return value from the
   :meth:`~object.\_\_exit\_\_` method was false, the exception is reraised. If the return
   value was true, the exception is suppressed, and execution continues with the
   statement following the :keyword:`with` statement.

   If the suite was exited for any reason other than an exception, the return
   value from :meth:`~object.\_\_exit\_\_` is ignored, and execution proceeds at the normal
...
for :meth:`~object.\_\_enter\_\_` and :meth:`~object.\_\_exit\_\_`.
...
1. The subject expression subject\_expr is evaluated and a resulting subject
   value obtained. If the subject expression contains a comma, a tuple is
   constructed using :ref:`the standard rules <typesseq-tuple>`.
2. Each pattern in a case\_block is attempted to match with the subject value. The
...
3. If the pattern succeeds, the corresponding guard (if present) is evaluated. In
   ...
   - If the guard evaluates as true or is missing, the block inside
     case\_block is executed.
   - Otherwise, the next case\_block is attempted as described above.
...
Only the final subpattern may be :ref:`irrefutable <irrefutable\_case>`, and each
...
capture\_pattern cannot be a \_.
...
The forms signed\_number '+' NUMBER and signed\_number '-' NUMBER are
...
:token:`~python-grammar:wildcard\_pattern`.
...
match subject expressions, guards, and case blocks.
...
:ref:`name resolution rules <resolve\_names>`. The pattern succeeds if the
...
1. If the subject value is not a sequence [[2]](#footnote-2), the sequence pattern
   fails.
...
3. The subsequent steps depend on whether the sequence pattern is fixed or
   ...
   :func:`len` (i.e. via the :meth:`~object.\_\_len\_\_` protocol).
...
1. If the subject value is not a mapping [[3]](#footnote-3),the mapping pattern fails.
...
in the mapping, and not created on-the-fly via :meth:`~object.\_\_missing\_\_`
or :meth:`~object.\_\_getitem\_\_`.
...
1. If name\_or\_attr is not an instance of the builtin :class:`type` , raise
   :exc:`TypeError`.
2. If the subject value is not an instance of name\_or\_attr (tested via
...
3. If no pattern arguments are present, the pattern succeeds. Otherwise,
   ...
   patterns using the :data:`~object.\_\_match\_args\_\_` attribute on the class
   name\_or\_attr before matching:

   1. The equivalent of getattr(cls, "\_\_match\_args\_\_", ()) is called.

      - If this raises an exception, the exception bubbles up.
      - If the returned value is not a tuple, the conversion fails and
        :exc:`TypeError` is raised.
      - If there are more positional patterns than len(cls.\_\_match\_args\_\_),
        :exc:`TypeError` is raised.
      - Otherwise, positional pattern i is converted to a keyword pattern
        using \_\_match\_args\_\_[i] as the keyword. \_\_match\_args\_\_[i] must
...
- convert P1 to a keyword pattern using CLS.\_\_match\_args\_\_
...
only when the function is called. [[4]](#footnote-4)
...
:attr:`~function.\_\_type\_params\_\_`
...
namespace is saved. [[5]](#footnote-5) A class object is then created using the inheritance
list for the base classes and the saved local namespace for the attribute
dictionary. The class name is bound to this class object in the original local
namespace.

The order in which attributes are defined in the class body is preserved
in the new class's :attr:`~type.\_\_dict\_\_`. Note that this is reliable only right
...
:attr:`~type.\_\_type\_params\_\_` attribute. See :ref:`generic-classes` for more.
...
searched when looking up an attribute on a class. See :ref:`python\_2.3\_mro` for a
...
An :term:`asynchronous iterable` provides an \_\_aiter\_\_ method that directly
...
for :meth:`~object.\_\_aiter\_\_` and :meth:`~object.\_\_anext\_\_`.
...
for :meth:`~object.\_\_aenter\_\_` and :meth:`~object.\_\_aexit\_\_`.
...
:attr:`~definition.\_\_type\_params\_\_` attribute listing their type parameters.
...
but only when the value is explicitly accessed through the attributes \_\_bound\_\_
and \_\_constraints\_\_. To accomplish this, the bounds or constraints are
...
object is created, but only when the type parameter's \_\_default\_\_ attribute
...
for a type parameter, the \_\_default\_\_ attribute is set to the special
...
The capitalized names like DEFAULT\_OF\_arg are not actually
bound at runtime.
...
TYPE\_PARAMS\_OF\_Bag is not actually bound at runtime.
...
like TYPE\_PARAMS\_OF\_ListOrSet are not actually bound at runtime.
...
If the :ref:`future statement <future>` from \_\_future\_\_ import annotations is present,
...
:func:`annotationlib.get\_annotations` and :func:`typing.get\_type\_hints` are
...
- a builtin class that has its (CPython) :c:macro:`Py\_TPFLAGS\_SEQUENCE` bit set
...
- a builtin class that has its (CPython) :c:macro:`Py\_TPFLAGS\_MAPPING` bit set
- a class that inherits from any of the above
...
transformed into the function's :attr:`~function.\_\_doc\_\_` attribute and
...
transformed into the namespace's :attr:`~type.\_\_doc\_\_` item and therefore
