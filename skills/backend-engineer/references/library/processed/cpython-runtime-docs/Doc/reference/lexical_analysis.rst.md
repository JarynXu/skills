...
> Upstream path: `Doc/reference/lexical_analysis.rst`  
> Upstream Git blob: `f3ed1539493b6addb286f4fd59dcebbd8a6d6308`  
...
| --- | --- |
| - space - tab - formfeed | - :ref:`Whitespace <whitespace>` |
| - CR, LF | - :ref:`New line <line-structure>` - :ref:`Indentation <indentation>` |
| - backslash (\) | - :ref:`Explicit line joining <explicit-joining>` - (Also significant in :ref:`string escape sequences <escape-sequences>`) |
| - hash (#) | - :ref:`Comment <comments>` |
| - quote (', ") | - :ref:`String literal <strings>` |
| - ASCII letter (a-z, A-Z) - non-ASCII character | - :ref:`Name <identifiers>` - Prefixed :ref:`string or bytes literal <strings>` |
| - underscore (\_) | - :ref:`Name <identifiers>` - (Can also be part of :ref:`numeric literals <numbers>`) |
| - number (0-9) | - :ref:`Numeric literal <numbers>` |
| - dot (.) | - :ref:`Numeric literal <numbers>` - :ref:`Operator <operators>` |
| - question mark (?) - dollar ($) - backquote (​`​) - control character | - Error (outside string literals and comments) |
| - other printing character | - :ref:`Operator or delimiter <operators>` |
| - end of file | - :ref:`End marker <endmarker-token>` |
...
:   System-defined names, informally known as "dunder" names. These names are
    ...
    More will likely be defined in future versions of Python. *Any* use of \_\_\*\_\_ names,
...
For example, ř\_1, 蛇, or साँप are valid names, but r〰2,
€, or 🐍 are not.
...
start with a character in the "letter-like" set xid\_start,
and the remaining characters must be in the "letter- and digit-like" set
xid\_continue.

These sets are based on the *XID\_Start* and *XID\_Continue* sets as defined by the
Unicode standard annex [UAX-31](https://www.unicode.org/reports/tr31/).
Python's xid\_start additionally includes the underscore (\_).
Note that Python does not necessarily conform to [UAX-31](https://www.unicode.org/reports/tr31/).

A non-normative listing of characters in the *XID\_Start* and *XID\_Continue*
...
For reference, the construction rules for the xid\_\* sets are given below.

The set id\_start is defined as the union of:
...
- {"\_"} - the underscore
- <Other\_ID\_Start> - an explicit set of characters in [PropList.txt](https://www.unicode.org/Public/17.0.0/ucd/PropList.txt)
  to support backwards compatibility

The set xid\_start then closes this set under NFKC normalization, by
removing all characters whose normalization is not of the form
id\_start id\_continue\*.

The set id\_continue is defined as the union of:
...
- <Other\_ID\_Continue> - another explicit set of characters in
  [PropList.txt](https://www.unicode.org/Public/17.0.0/ucd/PropList.txt) to support backwards compatibility

Again, xid\_continue closes this set under NFKC normalization.
...
| --- | --- |
| \<newline> | :ref:`string-escape-ignore` |
...
| :samp:`\\\\{ooo}` | :ref:`string-escape-oct` |
| :samp:`\\x{hh}` | :ref:`string-escape-hex` |
| :samp:`\\N\\{{name}\\}` | :ref:`string-escape-named` |
| :samp:`\\u{xxxx}` | :ref:`Hexadecimal Unicode character <string-escape-long-hex>` |
| :samp:`\\U{xxxxxxxx}` | :ref:`Hexadecimal Unicode character <string-escape-long-hex>` |
...
tokens :py:data:`~token.FSTRING\_START`, :py:data:`~token.FSTRING\_MIDDLE`
and :py:data:`~token.FSTRING\_END`, and partly by the parser, which handles
...
- There may be no whitespace in :py:data:`~token.FSTRING\_START` (between
  the prefix and quote).
- Whitespace in :py:data:`~token.FSTRING\_MIDDLE` is part of the literal
  string contents.
- In fstring\_replacement\_field, if f\_debug\_specifier is present,
  all whitespace after the opening brace until the f\_debug\_specifier,
  as well as whitespace immediately following f\_debug\_specifier,
  is retained as part of the expression.

The FSTRING\_MIDDLE definition uses
...
In the above grammar snippet, the f\_quote and FSTRING\_MIDDLE rules
are context-sensitive -- they depend on the contents of FSTRING\_START
...
For example, \_123, 321\_, and 123\_\_321 are *not* valid literals.
...
For example, 0x\_1f is a valid literal, but 0\_x1f and 0x\_\_1f are
not.
...
A :ref:`list of these tokens and their names <token\_operators\_delimiters>`
