...
> Upstream path: `docs/topics/testing/tools.txt`  
> Upstream Git blob: `944701cdef16b568ec411fd9176d4cbc572e932a`  
...
.. class:: Client(enforce_csrf_checks=False, raise_request_exception=True, json_encoder=DjangoJSONEncoder, *, headers=None, query_params=None, **defaults)
    ...
    .. method:: Client.alogin(**credentials)
        ...
        *Asynchronous version*: ``alogin()``
        ...
        ``login()`` returns ``True`` if the credentials were accepted and
...
* Access to the synchronous and asynchronous test clients via the
  :attr:`~SimpleTestCase.client` and :attr:`~SimpleTestCase.async_client`
  attributes.
...
.. attribute:: SimpleTestCase.async_client

Every ``django.test.*TestCase`` instance has access to Django's synchronous
and asynchronous test clients as ``self.client`` and ``self.async_client``,
respectively. Both clients are recreated for each test, so you don't have to
...
.. attribute:: SimpleTestCase.async_client_class

If you want to use a different :class:`Client` or :class:`AsyncClient` class
(for example, a subclass with customized behavior), use the
:attr:`~SimpleTestCase.client_class` or
:attr:`~SimpleTestCase.async_client_class` class attributes.
...
The same pattern applies to the asynchronous test client by subclassing
:class:`AsyncClient` and setting :attr:`~SimpleTestCase.async_client_class`.
...
:setting:`BACKEND <MAILERS-BACKEND>` for each configured :setting:`MAILERS`
alias with the ``'locmem'`` :ref:`memory email backend
<topic-email-memory-backend>`. (Don't worry -- this has no effect on any other
