...
> Upstream path: `doc/build/orm/session_transaction.rst`  
> Upstream Git blob: `55ade3e5326221756ecee859a8cd88b661ae983c`  
...
delineated using the :meth:`~.Session.begin\_nested`
method:
...
Each time :meth:`\_orm.Session.begin\_nested` is called, a new "BEGIN SAVEPOINT"
...
:meth:`\_orm.Session.begin\_nested` is typically used as a context manager
...
When the context manager yielded by :meth:`\_orm.Session.begin\_nested`
...
When :meth:`~.Session.begin\_nested` is called, the :class:`\_orm.Session` first
...
:meth:`\_orm.Session.begin\_nested` is rolled back, in-memory object state that
...
When using a SAVEPOINT via the :meth:`\_orm.Session.begin\_nested` or
:meth:`\_engine.Connection.begin\_nested` methods, the transaction object
...
that the :meth:`\_orm.SessionEvents.after\_transaction\_create` event is invoked
...
:func:`\_sa.create\_engine` level as well as at the :meth:`\_engine.Connection.execution\_options`
level.
...
levels at once, is to use the :meth:`\_engine.Engine.execution\_options` method,
...
Above, both "eng" and "autocommit\_engine" share the same dialect and
connection pool. However the "AUTOCOMMIT" mode will be set upon connections
when they are acquired from the autocommit\_engine. The two
:class:`\_orm.sessionmaker` objects "transactional\_session" and "autocommit\_session"
then inherit these characteristics when they work with database connections.

The "autocommit\_session" **continues to have transactional semantics**,
including that
...
can use the :meth:`.Session.bind\_mapper` or :meth:`.Session.bind\_table`
methods:
...
See the section :ref:`session\_transaction\_events` for an overview
...
:paramref:`\_orm.Session.join\_transaction\_mode` parameter is passed with the
setting "create\_savepoint", which indicates that new SAVEPOINTs should be
