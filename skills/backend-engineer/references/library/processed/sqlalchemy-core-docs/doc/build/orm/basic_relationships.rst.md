...
> Upstream path: `doc/build/orm/basic_relationships.rst`  
> Upstream Git blob: `a0a541fb982583f086baf7f57f08f948872bb888`  
...
using :ref:`Declarative <orm\_explicit\_declarative\_base>` style mappings
...
:ref:`Declarative <orm\_explicit\_declarative\_base>` mappings with
...
Finally, using :ref:`Imperative Mapping <orm\_imperative\_mapping>`, which
...
it using the :paramref:`\_orm.relationship.collection\_class` parameter:
...
the two using the :paramref:`\_orm.relationship.back\_populates` parameter,
using the attribute name of each :func:`\_orm.relationship`
as the value for :paramref:`\_orm.relationship.back\_populates` on the other:
...
:paramref:`\_orm.relationship.collection\_class` parameter.
...
the delete cascade option described at :ref:`cascade\_delete` is used.
...
behavior; the next section, :ref:`relationship\_patterns\_nullable\_m2o`,
illustrates a nullable version.

Bidirectional behavior is achieved by adding a second :func:`\_orm.relationship`
and applying the :paramref:`\_orm.relationship.back\_populates` parameter
...
allowing None; this follows from the Parent.child\_id column itself
not being nullable, as it is typed with Mapped[int]. If we wanted
Parent.child to be a **nullable** many-to-one, we can set both
Parent.child\_id and Parent.child to be Optional[] (or its
...
Above, the column for Parent.child\_id will be created in DDL to allow
NULL values. When using :func:`\_orm.mapped\_column` with explicit typing
declarations, the specification of child\_id: Mapped[Optional[int]] is
equivalent to setting :paramref:`\_schema.Column.nullable` to True on the
:class:`\_schema.Column`, whereas child\_id: Mapped[int] is equivalent to
setting it to False. See :ref:`orm\_declarative\_mapped\_column\_nullability`
...
One To One is essentially a :ref:`relationship\_patterns\_o2m`
...
setting the previous child.parent\_id column to NULL by default unless there
are specific :ref:`cascade <unitofwork\_cascades>` behaviors set up.
...
of more than one Child, unless the :paramref:`\_orm.relationship.single\_parent`
...
Whether or not :paramref:`\_orm.relationship.single\_parent` is used, it is
recommended that the database schema include a :ref:`unique constraint
<schema\_unique\_constraint>` to indicate that the Child.parent\_id column
should be unique, to ensure at the database level that only one Child row may refer
to a particular Parent row at a time (see :ref:`orm\_declarative\_table\_configuration`
for background on the \_\_table\_args\_\_ tuple syntax):
...
type of each of association.left\_id and association.right\_id is
...
collection. Specify using :paramref:`\_orm.relationship.back\_populates`, and
...
:ref:`orm\_declarative\_relationship\_secondary\_eval` for background and
examples.
...
to that of :ref:`relationship\_patterns\_o2m`, as described at
:ref:`relationship\_patterns\_o2m\_collection`. For an annotated mapping
...
- A higher performing option here is to use ON DELETE CASCADE directives
  ...
  collection in this case using the :paramref:`\_orm.relationship.passive\_deletes`
  directive on :func:`\_orm.relationship`; see :ref:`passive\_deletes` for more details
  on this.
...
deleted - see :ref:`unitofwork\_cascades` for information on this feature.
...
an additional column called extra\_data, which is a string value that
...
constructs, linked to the existing ones using :paramref:`\_orm.relationship.back\_populates`:
...
provides the :ref:`associationproxy\_toplevel` extension. This
...
Avoid mixing the association object pattern with the :ref:`many-to-many <relationships\_many\_to\_many>`
...
the :ref:`association proxy <associationproxy\_toplevel>` is typically
...
:ref:`association\_pattern\_w\_m2m`.
...
between Parent.child\_associations -> Association.child
and Child.parent\_associations -> Association.parent:
...
Parent.child\_associations or Child.parent\_associations in Python;
...
value for the association.extra\_data column, which will receive
NULL for its value.
...
that's read from Parent.child\_associations or Child.parent\_associations,
...
view. This extension is known as the :ref:`Association Proxy <associationproxy\_toplevel>`.
...
- :paramref:`\_orm.relationship.order\_by`
- :paramref:`\_orm.relationship.primaryjoin`
- :paramref:`\_orm.relationship.secondaryjoin`
- :paramref:`\_orm.relationship.secondary`
- :paramref:`\_orm.relationship.remote\_side`
- :paramref:`\_orm.relationship.foreign\_keys`
- :paramref:`\_orm.relationship.\_user\_defined\_foreign\_keys`
...
:ref:`orm\_declarative\_table\_adding\_columns`, any :class:`\_orm.MapperProperty`
...
dynamically generated class returned by :func:`\_orm.declarative\_base`
or :meth:`\_orm.registry.generate\_base`. This "base" class includes
a Python metaclass which implements a special \_\_setattr\_\_() method
...
or imperative functions like :meth:`\_orm.registry.map\_imperatively`.
...
For the example given at :ref:`relationships\_many\_to\_many`, if we assumed
that the association\_table :class:`.Table` object would be defined at a point later on in the
...
"association\_table" is evaluated as a variable
named "association\_table" that is resolved against the table names within
