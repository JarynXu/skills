...
> Upstream path: `doc/build/orm/cascades.rst`  
> Upstream Git blob: `62b0168f1aa8249497c03a321d9395b4b3a22cdc`  
...
so-called :ref:`cascade\_save\_update` and :ref:`cascade\_merge` settings.
The typical "alternative" setting for cascade is to add
the :ref:`cascade\_delete` and :ref:`cascade\_delete\_orphan` options;
...
:ref:`cascade\_refresh\_expire`
cascade setting which may not be desirable when using the
:ref:`asyncio\_toplevel` extension, as it will expire related objects
more aggressively than is typically appropriate in an explicit IO context.
See the notes at :ref:`asyncio\_orm\_avoid\_lazyloads` for further background.
...
the :paramref:`\_orm.relationship.back\_populates` or :paramref:`\_orm.relationship.backref`
...
as cascade\_backrefs. Finally, in SQLAlchemy 1.4 the old behavior was
deprecated and the cascade\_backrefs option was removed in SQLAlchemy 2.0.
...
:ref:`cascade\_delete` cascade on one-to-many relationships is often combined
with :ref:`cascade\_delete\_orphan` cascade, which will emit a DELETE for the
...
directives described at :ref:`passive\_deletes` should be used.
...
:ref:`orm\_queryguide\_update\_delete\_where`. See
:ref:`orm\_queryguide\_update\_delete\_caveats` for additional background.
...
The following example adapts that of :ref:`relationships\_many\_to\_many` to
...
metadata is described at :ref:`on\_update\_on\_delete`.
...
the :paramref:`\_orm.relationship.passive\_deletes` parameter and it accepts
...
1. The application calls session.delete(my\_parent), where my\_parent
   is an instance of Parent.
2. When the :class:`\_orm.Session` next flushes changes to the database,
   all of the **currently loaded** items within the my\_parent.children
   collection are deleted by the ORM, meaning a DELETE statement is
   emitted for each record.
3. If the my\_parent.children collection is **unloaded**, then no DELETE
   statements are emitted. If the :paramref:`\_orm.relationship.passive\_deletes`
...
4. A DELETE statement is then emitted for the my\_parent row itself.
...
6. The Parent instance referred to by my\_parent, as well as all
...
- When using MySQL, an appropriate storage engine must be
  selected. See :ref:`mysql\_storage\_engines` for details.
- When using SQLite, foreign key support must be enabled explicitly.
  See :ref:`sqlite\_foreign\_keys` for details.
...
- Database level foreign keys with no ON DELETE setting are often used
  ...
  >   :paramref:`\_orm.relationship.passive\_deletes` flag to the string
...
- SQLAlchemy doesn't **need** to be this sophisticated, as we instead
  ...
  functionality, by using the :paramref:`\_orm.relationship.passive\_deletes`
  ...
  emitting a SELECT for them. The section :ref:`passive\_deletes` provides
...
- While database-level ON DELETE functionality works only on the "many"
  ...
  :paramref:`\_orm.relationship.single\_parent` flag should be used to
  establish an in-Python assertion for this case.

### Using foreign key ON DELETE with many-to-many relationships

As described at :ref:`cascade\_delete\_many\_to\_many`, "delete" cascade works
...
In this case, the :paramref:`\_orm.relationship.passive\_deletes` directive can
...
passive\_deletes=True on the **other** side of the bidirectional
...
4. for each Child object affected by this immediate deletion, because
   passive\_deletes=True is configured, the unit of work will not need to
...
a single object at a time by configuring the :paramref:`\_orm.relationship.single\_parent` argument,
...
:paramref:`\_orm.relationship.single\_parent` which invokes an assertion
