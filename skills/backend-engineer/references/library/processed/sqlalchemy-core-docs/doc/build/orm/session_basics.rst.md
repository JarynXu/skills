...
> Upstream path: `doc/build/orm/session_basics.rst`  
> Upstream Git blob: `0c04e34b2ed23c147270fe14823ed83286b4e49c`  
...
new :class:`.Session`, unless the :paramref:`.Session.expire\_on\_commit`
parameter is used to disable this behavior. See the
section :ref:`session\_committing` for more detail.
...
:class:`\_engine.Engine` object created by :func:`\_sa.create\_engine`, which
...
:ref:`queryguide\_toplevel`. Some brief examples follow:
...
this works in the section :ref:`unitofwork\_cascades`, but in general
the rules are:
...
- To change the "SET NULL" into a DELETE of a related object's row, use the
  :ref:`cascade\_delete` cascade on the :func:`\_orm.relationship`.
...
- When related objects include a foreign key constraint back to the object
  ...
  objects.
- The :paramref:`\_orm.relationship.passive\_deletes` parameter can be used
  ...
  or DELETE. Setting :paramref:`\_orm.relationship.passive\_deletes` to
...
- When the DELETE occurs for an object marked for deletion, the object
  ...
  and then :ref:`cascade\_delete\_orphan` should be used so that it is
  deleted as a secondary effect of that collection removal. See the
  section :ref:`session\_deleting\_from\_collections` for an example of this.
...
:meth:`~.Session.begin\_nested` is used.
...
- When objects are :ref:`refreshed <session\_expiring>`
...
- When :meth:`.Session.begin\_nested` is called
...
:attr:`.Session.no\_autoflush` context manager:
...
methods such as :meth:`.Session.commit` and :meth:`.Session.begin\_nested` are
...
:ref:`isolation level <session\_transaction\_isolation>` of the database
transaction), provided that the DBAPI is not in
:ref:`driver level autocommit <dbapi\_autocommit>` mode. This means that
...
:ref:`faq\_session\_rollback` contains a more detailed description of this
behavior.
...
from the database transaction. The FAQ entry at :ref:`faq\_session\_identity`
...
- **the refresh() method** - closely related is the :meth:`\_orm.Session.refresh`
  ...
  ```
- **the populate\_existing() method or execution option** - This is now
  an execution option documented at :ref:`orm\_queryguide\_populate\_existing`; in
  legacy form it's found on the :class:`\_orm.Query` object as the
  :meth:`\_orm.Query.populate\_existing` method. This operation in either form
...
:meth:`\_orm.Session.in\_transaction` method, which returns True or False
indicating if the "autobegin" step has proceeded. While not normally needed,
the :meth:`\_orm.Session.get\_transaction` method will return the actual
...
manager as described at :ref:`session\_begin\_commit\_rollback\_block`.
...
at :ref:`Partitioning Strategies <session\_partitioning>`), the same COMMIT
...
unless :ref:`two-phase features <session\_twophase>` are enabled.
...
:ref:`session\_external\_transaction` for background on this pattern.
...
controlled by the :paramref:`\_orm.Session.expire\_on\_commit` flag, which may be
...
been begun either via :ref:`autobegin <session\_autobegin>`
...
>   behavior noted in :ref:`session\_committing`, where
...
>   at :ref:`session\_external\_transaction`, rollback behavior on this
>   :class:`\_engine.Connection` would follow the behavior specified by the
>   :paramref:`\_orm.Session.join\_transaction\_mode` parameter, which could
...
>   :paramref:`\_orm.Session.expire\_on\_commit` setting.
...
closed and discarded). See the FAQ entry at :ref:`faq\_session\_rollback` for
further discussion.

### Closing

The :meth:`~.Session.close` method issues a :meth:`~.Session.expunge\_all` which
...
parameter :paramref:`\_orm.Session.close\_resets\_only` to False, indicating that
...
:paramref:`\_orm.Session.close\_resets\_only` is set to True.
...
place the :class:`.sessionmaker` line in your \_\_init\_\_.py file; from
...
- Web applications. In this case, it's best to make use of the SQLAlchemy
  ...
  :paramref:`\_orm.Session.expire\_on\_commit` to False so that subsequent
...
session.scalars(select(Foo).filter\_by(name='bar')), even if Foo(name='bar')
...
via the :ref:`examples\_caching` example.

### How can I get the :class:`~sqlalchemy.orm.session.Session` for a certain object?

Use the :meth:`~.Session.object\_session` classmethod
...
The newer :ref:`core\_inspection\_toplevel` system can also be used:
...
:ref:`asyncio <asyncio\_toplevel>` extension, this object is only a thin proxy
...
pattern <session\_getting>` locally within the top level Python function that
...
functions and methods which require it, the :class:`.scoped\_session`
approach can provide for a "thread local" :class:`.Session` object;
see the section :ref:`unitofwork\_contextual` for background. Within
the asyncio context, the :class:`.async\_scoped\_session`
object is the asyncio analogue for :class:`.scoped\_session`, however is more
