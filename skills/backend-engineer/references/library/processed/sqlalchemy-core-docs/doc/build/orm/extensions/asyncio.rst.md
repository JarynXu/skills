...
> Upstream path: `doc/build/orm/extensions/asyncio.rst`  
> Upstream Git blob: `6985c4c34cb24bbc8fd1cd3c2cb92bddf1a93a8d`  
...
Please read :ref:`asyncio\_install` for important platform
...
For Core use, the :func:`\_asyncio.create\_async\_engine` function creates an
...
Above, the :meth:`\_asyncio.AsyncConnection.run\_sync` method may be used to
invoke special DDL functions such as :meth:`\_schema.MetaData.create\_all` that
don't include an awaitable hook.

Tip

It's advisable to invoke the :meth:`\_asyncio.AsyncEngine.dispose` method
...
async\_main function in the above example. This ensures that any
...
cannot properly dispose of these connections within methods like \_\_del\_\_
...
column attributes; the next section :ref:`asyncio\_orm\_avoid\_lazyloads` details
this.
...
:ref:`asyncio\_concurrency` and :ref:`session\_faq\_threadsafe` for background.
...
the optional :class:`\_asyncio.async\_sessionmaker` helper, which provides
...
See the section :ref:`session\_faq\_threadsafe` for a general description of
...
- Attributes that are lazy-loading relationships, deferred columns or
  ...
  provides an accessor :attr:`\_asyncio.AsyncAttrs.awaitable\_attrs`
  ...
  as an awaitable by indicating the :attr:`\_asyncio.AsyncAttrs.awaitable\_attrs`
  prefix:
...
- The :class:`\_asyncio.AsyncSession` is configured using
  :paramref:`\_orm.Session.expire\_on\_commit` set to False, so that we may access
...
- Methods like :meth:`\_asyncio.AsyncSession.expire` should be avoided in favor of
  ...
  :paramref:`\_orm.Session.expire\_on\_commit`
...
- A lazy-loaded relationship **can be loaded explicitly under asyncio** using
  ...
  :paramref:`\_orm.Session.refresh.attribute\_names`, e.g.:
...
- Avoid using the all cascade option documented at :ref:`unitofwork\_cascades`
  in favor of listing out the desired cascade features explicitly. The
  all cascade option implies among others the :ref:`cascade\_refresh\_expire`
...
- Appropriate loader options should be employed for :func:`\_orm.deferred`
  ...
  constructs as noted above. See :ref:`orm\_queryguide\_column\_deferral` for
  background on deferred column loading.

- The "dynamic" relationship loader strategy described at
  :ref:`dynamic\_relationship` is not compatible by default with the asyncio approach.
  It can be used directly only if invoked within the
  :meth:`\_asyncio.AsyncSession.run\_sync` method described at
  :ref:`session\_run\_sync`, or by using its .statement attribute
  ...
  The :ref:`write only <write\_only\_relationship>` technique, introduced in
...
- If using asyncio with a database that does not support RETURNING, such as
  ...
  :paramref:`\_orm.Mapper.eager\_defaults` option is used. In SQLAlchemy 2.0,
...
:meth:`\_asyncio.AsyncSession.run\_sync` is provided which will run any
...
invoked using :meth:`\_asyncio.AsyncSession.run\_sync`.
...
The SQLAlchemy :ref:`event system <event\_toplevel>` is not directly exposed
...
- Events can be registered at the instance level (e.g. a specific
  ...
  :attr:`\_asyncio.AsyncEngine.sync\_engine` attribute as target. Targets
  include:

  > :attr:`\_asyncio.AsyncEngine.sync\_engine`
  >
  > :attr:`\_asyncio.AsyncConnection.sync\_connection`
  >
  > :attr:`\_asyncio.AsyncConnection.sync\_engine`
  >
  > :attr:`\_asyncio.AsyncSession.sync\_session`
- To register an event at the class level, targeting all instances of the same type (e.g.
  ...
  :meth:`\_ormevents.SessionEvents.before\_commit` event against the
...
- To register at the :class:`\_orm.sessionmaker` level, combine an explicit
  :class:`\_orm.sessionmaker` with an :class:`\_asyncio.async\_sessionmaker`
  using :paramref:`\_asyncio.async\_sessionmaker.sync\_session\_class`, and
...
- **Core Events on AsyncEngine**
  ...
  In this example, we access the :attr:`\_asyncio.AsyncEngine.sync\_engine`
...
- **ORM Events on AsyncSession**
  ...
  In this example, we access :attr:`\_asyncio.AsyncSession.sync\_session` as the
  ...
  after commit!
  ```
- **ORM Events on async\_sessionmaker**

  For this use case, we make a :class:`\_orm.sessionmaker` as the event target,
  then assign it to the :class:`\_asyncio.async\_sessionmaker` using
  the :paramref:`\_asyncio.async\_sessionmaker.sync\_session\_class` parameter:
...
driver connection. One such example is the .set\_type\_codec() method
supplied by the asyncpg driver.

To accommodate this use case, SQLAlchemy's :class:`.AdaptedConnection`
class provides a method :meth:`.AdaptedConnection.run\_async` that allows
...
analogous to the :meth:`\_asyncio.AsyncConnection.run\_sync` method that
allows a sync-style method to run under async.

:meth:`.AdaptedConnection.run\_async` should be passed a function that will
accept the innermost "driver" connection as a single argument, and return
an awaitable that will be invoked by the :meth:`.AdaptedConnection.run\_async`
...
Above, the object passed to the register\_custom\_types event handler
...
The :meth:`.AdaptedConnection.run\_async` method then provides access to an
...
:class:`.scoped\_session` object is also available in asyncio, using
an adapted version called :class:`\_asyncio.async\_scoped\_session`.
...
When using :class:`\_asyncio.async\_scoped\_session`, as there's no "thread-local"
...
asyncio.current\_task() function for this purpose:
...
The "scopefunc" used by :class:`\_asyncio.async\_scoped\_session`
...
Using current\_task() for the "key" in the scope requires that
the :meth:`\_asyncio.async\_scoped\_session.remove` method is called from
...
the correct use of :meth:`\_asyncio.async\_scoped\_session.remove`.

:class:`\_asyncio.async\_scoped\_session` includes **proxy
behavior** similar to that of :class:`.scoped\_session`, which means it can be
...
:meth:`\_asyncio.async\_scoped\_session.remove` method:
...
:class:`\_reflection.Inspector` (introduced at :ref:`metadata\_reflection\_inspector`),
however the existing interface may be used in an asyncio context by
leveraging the :meth:`\_asyncio.AsyncConnection.run\_sync` method of
