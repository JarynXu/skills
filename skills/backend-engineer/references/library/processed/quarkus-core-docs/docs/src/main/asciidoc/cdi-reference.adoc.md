...
> Upstream path: `docs/src/main/asciidoc/cdi-reference.adoc`  
> Upstream Git blob: `511dbfe3459abc0b35b750c358447612883108ca`  
...
TIP: If you're new to CDI we recommend you to read the [Introduction to CDI](cdi.adoc) first.

TIP: The [CDI integration guide](cdi-integration.adoc) has more detail on common CDI-related integration use cases, and example code for solutions.
...
In Quarkus, we have [Qute](qute-reference.adoc#injecting-beans-directly-in-templates).
...
One of the limitations of GraalVM is the usage of [Reflection, window="_blank"](https://www.graalvm.org/{graalvm-docs-version}/reference-manual/native-image/dynamic-features/Reflection/).
...
* `@SessionScoped`
** Only with the Undertow extension; see [here](cdi.adoc#bean-scope-available) for details
...
* Annotate the bean with `@io.quarkus.runtime.Startup` as described in [Startup annotation](lifecycle.adoc#startup_annotation):
+
```java
@Startup // <1>
@ApplicationScoped
...
NOTE: Quarkus users are encouraged to always prefer the `@Observes StartupEvent` to `@Initialized(ApplicationScoped.class)` as explained in the [Application Initialization and Termination](lifecycle.adoc) guide.
...
As documented in [the `@Startup` method documentation](lifecycle.adoc#startup_annotation), you may define startup
...
Nevertheless, most of the functionality can be achieved using Quarkus [extensions](writing-extensions.adoc).
See the [integration guide](cdi-integration.adoc) for more information.
