...
> Upstream path: `docs/src/main/asciidoc/config-reference.adoc`  
> Upstream Git blob: `08023fb111610dc387d9c70c64f566f65e937118`  
...
[MicroProfile Config](https://github.com/eclipse/microprofile-config/blob/master/spec/src/main/asciidoc/configsources.asciidoc#default-configsources).
...
* [YAML](config-yaml.adoc)
* https://github.com/quarkiverse/quarkus-vault[HashiCorp Vault]
* https://github.com/quarkiverse/quarkus-config-extensions[Consul]
* [Spring Cloud](spring-cloud-config-client.adoc)

TIP: It is also possible to create a [Custom Config Source](config-extending-support.adoc#custom-config-source).
...
TIP: Check the [Mapping configuration to objects](config-mappings.adoc) guide for advanced usages with
`@ConfigMapping`.
...
TIP: Use [Config Mappings](config-mappings.adoc#config-mappings) to group similar configuration properties.
...
[Secret Keys](https://smallrye.io/smallrye-config/Main/config/secret-keys/) documentation.
...
`application.properties`. All the Quarkus configuration properties are [documented and searchable](all-config.adoc).
...
TIP: The properties fixed at build time are marked with a lock icon (icon:lock[]) in the [list of all configuration options](all-config.adoc).
...
then check out how [re-augmentation](reaugmentation.adoc) can be used to rebuild the augmentation output for a
...
Quarkus collects the config property values injected in CDI beans during [static initialization phase](writing-extensions.adoc#bootstrap-three-phases).
...
* [YAML ConfigSource Extension](config-yaml.adoc)
...
* [Spring Cloud ConfigSource Extension](spring-cloud-config-client.adoc)
* [Mapping configuration to objects](config-mappings.adoc)
* [Extending configuration support](config-extending-support.adoc)

Quarkus relies on [SmallRye Config](https://github.com/smallrye/smallrye-config/) and inherits its features:

* [Additional Config Sources](https://smallrye.io/smallrye-config/Main/config-sources/custom/)
* [Config Source Factories](https://smallrye.io/smallrye-config/Main/config-sources/factories/)
* [Additional Converters](https://smallrye.io/smallrye-config/Main/converters/custom/)
* [Interceptors](https://smallrye.io/smallrye-config/Main/extensions/interceptors/)
* [Relocate Configuration](https://smallrye.io/smallrye-config/Main/extensions/relocate/)
* [Fallback Configuration](https://smallrye.io/smallrye-config/Main/extensions/fallback/)
