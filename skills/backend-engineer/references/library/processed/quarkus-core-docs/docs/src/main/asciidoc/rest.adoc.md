...
> Upstream path: `docs/src/main/asciidoc/rest.adoc`  
> Upstream Git blob: `71aa6258133b7c86c24c7aa26440402c45da6b22`  
...
For a more lightweight introduction, please refer to the [Writing JSON REST services guides](rest-json.adoc).
====

## What is Quarkus REST?

Quarkus REST is a new [Jakarta REST (formerly known as JAX-RS)]({jaxrsspec})
...
.pom.xml
```xml,role="primary asciidoc-tabs-target-sync-cli asciidoc-tabs-target-sync-maven"
<dependency>
...
.build.gradle
```gradle,role="secondary asciidoc-tabs-target-sync-gradle"
...
Any class annotated with a [`@Path`]({jaxrsapi}/jakarta/ws/rs/Path.html) annotation can have its methods exposed as REST endpoints,
provided they have an HTTP method annotation (see below).

That [`@Path`]({jaxrsapi}/jakarta/ws/rs/Path.html) annotation defines the URI prefix under which those methods will be exposed. It can
be empty, or contain a prefix such as `rest` or `rest/V1`.

Each exposed endpoint method can in turn have another [`@Path`]({jaxrsapi}/jakarta/ws/rs/Path.html) annotation which adds to its containing
...
|[`@GET`]({jaxrsapi}/jakarta/ws/rs/GET.html)
|Obtain a resource representation, should not modify state, [idempotent]({httpspec}#section-4.2.2) ([HTTP docs]({httpspec}#section-4.3.1))

|[`@HEAD`]({jaxrsapi}/jakarta/ws/rs/HEAD.html)
|Obtain metadata about a resource, similar to `GET` with no body ([HTTP docs]({httpspec}#section-4.3.2))

|[`@POST`]({jaxrsapi}/jakarta/ws/rs/POST.html)
|Create a resource and obtain a link to it ([HTTP docs]({httpspec}#section-4.3.3))

|[`@PUT`]({jaxrsapi}/jakarta/ws/rs/PUT.html)
|Replace a resource or create one, should be [idempotent]({httpspec}#section-4.2.2) ([HTTP docs]({httpspec}#section-4.3.4))

|[`@DELETE`]({jaxrsapi}/jakarta/ws/rs/DELETE.html)
|Delete an existing resource, [idempotent]({httpspec}#section-4.2.2)  ([HTTP docs]({httpspec}#section-4.3.5))

|[`@OPTIONS`]({jaxrsapi}/jakarta/ws/rs/OPTIONS.html)
|Obtain information about a resource, [idempotent]({httpspec}#section-4.2.2) ([HTTP docs]({httpspec}#section-4.3.7))

|[`@PATCH`]({jaxrsapi}/jakarta/ws/rs/PATCH.html)
|Update a resource, or create one, not [idempotent]({httpspec}#section-4.2.2) (https://tools.ietf.org/html/rfc5789#section-2[HTTP docs])

|===

You can also declare other HTTP methods by declaring them as an annotation with the
[`@HttpMethod`]({jaxrsapi}/jakarta/ws/rs/HttpMethod.html) annotation:
...
the HTTP [`Content-Type`]({httpspec}#section-3.1.1.5) header, which in turn contains
[MIME (Media Type)]({httpspec}#section-3.1.1.1) values, such as the following:

- `text/plain` which is the default for any endpoint returning a `String`.
- `text/html` for HTML (such as with [Qute templating](qute.adoc))
...
You may annotate your endpoint class with the [`@Produces`]({jaxrsapi}/jakarta/ws/rs/Produces.html)
or [`@Consumes`]({jaxrsapi}/jakarta/ws/rs/Consumes.html) annotations, which
...
Any method may also be annotated with the [`@Produces`]({jaxrsapi}/jakarta/ws/rs/Produces.html)
or [`@Consumes`]({jaxrsapi}/jakarta/ws/rs/Consumes.html) annotations, in which
case they override any eventual class annotation.

The [`MediaType`]({jaxrsapi}/jakarta/ws/rs/core/MediaType.html) class has many constants you
...
|[`@RestPath`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestPath.html) (or nothing)
...
|Query parameter
|[`@RestQuery`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestQuery.html)
|The value of a https://tools.ietf.org/html/rfc3986#section-3.4[URI query parameter]

|Header
|[`@RestHeader`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestHeader.html)
|The value of an https://tools.ietf.org/html/rfc7230#section-3.2[HTTP header]

|Cookie
|[`@RestCookie`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestCookie.html)
|The value of an https://tools.ietf.org/html/rfc6265#section-4.2[HTTP cookie]

|Form parameter
|[`@RestForm`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestForm.html)
|The value of an https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/POST[HTTP URL-encoded FORM]

|Matrix parameter
|[`@RestMatrix`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestMatrix.html)
...
NOTE: The [`@RestPath`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestPath.html)
annotation is optional: any parameter whose name matches an existing URI
template variable will be automatically assumed to have [`@RestPath`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestPath.html).

You can also use any of the Jakarta REST annotations [`@PathParam`]({jaxrsapi}/jakarta/ws/rs/PathParam.html),
[`@QueryParam`]({jaxrsapi}/jakarta/ws/rs/QueryParam.html),
[`@HeaderParam`]({jaxrsapi}/jakarta/ws/rs/HeaderParam.html),
[`@CookieParam`]({jaxrsapi}/jakarta/ws/rs/CookieParam.html),
[`@FormParam`]({jaxrsapi}/jakarta/ws/rs/FormParam.html) or
[`@MatrixParam`]({jaxrsapi}/jakarta/ws/rs/MatrixParam.html) for this,
...
|[`String`]({jdkapi}/java/lang/String.html)
|The entire request body, decoded

|[`InputStream`]({jdkapi}/java/io/InputStream.html)
|The request body in a blocking stream

|[`Reader`]({jdkapi}/java/io/Reader.html)
|The request body in a blocking stream

|All Java primitives and their wrapper classes
|Java primitive types

|[`BigDecimal`]({jdkapi}/java/math/BigDecimal.html), [`BigInteger`]({jdkapi}/java/math/BigInteger.html)
|Large integers and decimals.

|[`JsonArray`]({jsonpapi}/jakarta/json/JsonArray.html), [`JsonObject`]({jsonpapi}/jakarta/json/JsonObject.html),
[`JsonStructure`]({jsonpapi}/jakarta/json/JsonStructure.html), [`JsonValue`]({jsonpapi}/jakarta/json/JsonValue.html)
|JSON value types

|[`Buffer`]({vertxapi}/io/vertx/core/buffer/Buffer.html)
|Vert.x Buffer
...
[`@RestForm`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestForm.html) annotation, but we have special types
...
[`@RestForm`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestForm.html) does not define a value, the field name is used),
...
NOTE: [`FileUpload`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/multipart/FileUpload.html)
...
NOTE: [`@PartType`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/PartType.html) is used to aid
...
As part of reading the multipart body, Quarkus REST invokes the proper MessageBodyReader[`MessageBodyReader`]({jaxrsapi}/jakarta/ws/rs/ext/MessageBodyReader.html) for each part of the request.
...
|[`Path`]({jdkapi}/java/nio/file/Path.html)
|The contents of the file specified by the given path

|[`PathPart`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/PathPart.html)
|The partial contents of the file specified by the given path

|[`FilePart`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/FilePart.html)
|The partial contents of a file

|[`AsyncFile`]({vertxapi}/io/vertx/core/file/AsyncFile.html)
|Vert.x AsyncFile, which can be in full, or partial

|===

Alternately, you can also return a <<reactive,reactive type>> such as [`Uni`]({mutinyapi}/io/smallrye/mutiny/Uni.html),
[`Multi`]({mutinyapi}/io/smallrye/mutiny/Multi.html) or
[`CompletionStage`]({jdkapi}/java/util/concurrent/CompletionStage.html)
...
NOTE: You can also use the Jakarta REST type [`Response`]({jaxrsapi}/jakarta/ws/rs/core/Response.html) but it is
...
[`Uni`]({mutinyapi}/io/smallrye/mutiny/Uni.html) type (from https://smallrye.io/smallrye-mutiny/[Mutiny]), in which
case the current HTTP request will be automatically suspended after your method, until
the returned [`Uni`]({mutinyapi}/io/smallrye/mutiny/Uni.html) instance resolves to a value,
...
The [`CompletionStage`]({jdkapi}/java/util/concurrent/CompletionStage.html) return
...
[`Multi`]({mutinyapi}/io/smallrye/mutiny/Multi.html) type (from https://smallrye.io/smallrye-mutiny/[Mutiny]).
...
[`@Produces(MediaType.SERVER_SENT_EVENTS)`]({jaxrsapi}/jakarta/ws/rs/Produces.html)
...
Quarkus REST provides the [`@Cache`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/Cache.html)
and [`@NoCache`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/NoCache.html) annotations to facilitate
...
While [`@Cache`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/Cache.html)
builds a complex `Cache-Control` header, [`@NoCache`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/NoCache.html)
is a simplified notation to say that you don't want anything cached; i.e. `Cache-Control: nocache`.

NOTE: More information on the `Cache-Control` header and be found in [RFC 7234](https://datatracker.ietf.org/doc/html/rfc7234)
...
|[`HttpHeaders`]({jaxrsapi}/jakarta/ws/rs/core/HttpHeaders.html)
|All the request headers

|[`ResourceInfo`]({jaxrsapi}/jakarta/ws/rs/container/ResourceInfo.html)
|Information about the current endpoint method and class (requires reflection)

|[`SecurityContext`]({jaxrsapi}/jakarta/ws/rs/core/SecurityContext.html)
|Access to the current user and roles

|[`SimpleResourceInfo`]({resteasy-reactive-api}/org/jboss/resteasy/reactive/server/SimpleResourceInfo.html)
|Information about the current endpoint method and class (no reflection required)

|[`UriInfo`]({jaxrsapi}/jakarta/ws/rs/core/UriInfo.html)
|Provides information about the current endpoint and application URI

|[`Application`]({jaxrsapi}/jakarta/ws/rs/core/Application.html)
|Advanced: Current Jakarta REST application class

|[`Configuration`]({jaxrsapi}/jakarta/ws/rs/core/Configuration.html)
|Advanced: Configuration about the deployed Jakarta REST application

|[`Providers`]({jaxrsapi}/jakarta/ws/rs/ext/Providers.html)
|Advanced: Runtime access to Jakarta REST providers

|[`Request`]({jaxrsapi}/jakarta/ws/rs/core/Request.html)
|Advanced: Access to the current HTTP method and <<preconditions>>

|[`ResourceContext`]({jaxrsapi}/jakarta/ws/rs/core/ResourceContext.html)
|Advanced: access to instances of endpoints

|[`ServerRequestContext`]({resteasy-reactive-api}/org/jboss/resteasy/reactive/server/spi/ServerRequestContext.html)
|Advanced: Quarkus REST access to the current request/response

|[`Sse`]({jaxrsapi}/jakarta/ws/rs/sse/Sse.html)
|Advanced: Complex SSE use-cases

|[`HttpServerRequest`]({vertxapi}/io/vertx/core/http/HttpServerRequest.html)
|Advanced: Vert.x HTTP Request

|[`HttpServerResponse`]({vertxapi}/io/vertx/core/http/HttpServerResponse.html)
...
```
<1> The `io.quarkus.resteasy.reactive.jackson.SecureField.rolesAllowed` property supports [property expressions](config-reference.adoc#property-expressions)
exactly in the same fashion the `jakarta.annotation.security.RolesAllowed` annotation does. For more information, please
refer to the [Standard security annotations](security-authorize-web-endpoints-reference.adoc#standard-security-annotations)
...
Assuming security has been set up for the application (see our [guide](security-overview.adoc) for more details), when a user with the `admin` role
...
To align with this approach, configure your application to minimize reflection by using the [`quarkus-rest-jackson`](https://quarkus.io/extensions/io.quarkus/quarkus-rest-jackson) extension.
...
[Cross-origin resource sharing](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing) (CORS) is a mechanism that
...
For more information about the CORS filters and their usage, see the [CORS filter](security-cors.adoc#cors-filter) section of the Quarkus "Cross-origin resource sharing" guide.
...
or any of the [Quarkus Reactive extensions](quarkus-reactive-architecture.adoc#quarkus-extensions-enabling-reactive) for example:
...
framework using [`WebApplicationException`]({jaxrsapi}/jakarta/ws/rs/WebApplicationException.html) or any of its subtypes:
...
[`@ServerExceptionMapper`]({resteasy-reactive-api}/org/jboss/resteasy/reactive/server/ServerExceptionMapper.html)
annotation on a method, with one parameter of the exception type you want to handle, and turning
that exception into a [`RestResponse`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestResponse.html) (or a
[`Uni<RestResponse<?>>`]({mutinyapi}/io/smallrye/mutiny/Uni.html)):
...
You can also declare [exception mappers in the Jakarta REST way]({jaxrsspec}#exceptionmapper).
====
...
|[`ContainerRequestContext`]({jaxrsapi}/jakarta/ws/rs/container/ContainerRequestContext.html)
...
|[`RestResponse`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestResponse.html) or [`Response`]({jaxrsapi}/jakarta/ws/rs/core/Response.html)
|The response to send to the client when the exception occurs

|[`Uni<RestResponse>`]({mutinyapi}/io/smallrye/mutiny/Uni.html) or [`Uni<Response>`]({mutinyapi}/io/smallrye/mutiny/Uni.html)
...
[`@ServerRequestFilter`]({resteasy-reactive-api}/org/jboss/resteasy/reactive/server/ServerRequestFilter.html)
annotation:
...
[`@ServerResponseFilter`]({resteasy-reactive-api}/org/jboss/resteasy/reactive/server/ServerResponseFilter.html)
annotation:
...
|[`ContainerResponseContext`]({jaxrsapi}/jakarta/ws/rs/container/ContainerResponseContext.html)
|A context object to access the current response

|[`Throwable`]({jdkapi}/java/lang/Throwable.html)
...
|[`Optional<RestResponse<?>>`]({jdkapi}/java/util/Optional.html) or [`Optional<Response>`]({jdkapi}/java/util/Optional.html)
...
NOTE: You can restrict the Resource methods for which a filter runs, by using [`@NameBinding`]({jaxrsapi}/jakarta/ws/rs/NameBinding.html) meta-annotations.

#### The Jakarta REST way

You can also [declare request and response filters in the Jakarta REST way]({jaxrsspec}#filters).
...
[`RestResponse<?>`]({resteasy-reactive-common-api}/org/jboss/resteasy/reactive/RestResponse.html)
or [`Response`]({jaxrsapi}/jakarta/ws/rs/core/Response.html) with
...
This is done via a pluggable system of [`MessageBodyReader`]({jaxrsapi}/jakarta/ws/rs/ext/MessageBodyReader.html)
and [`MessageBodyWriter`]({jaxrsapi}/jakarta/ws/rs/ext/MessageBodyWriter.html) interfaces,
...
with [`@Provider`]({jaxrsapi}/jakarta/ws/rs/ext/Provider.html):
...
[`ServerMessageBodyWriter`]({resteasy-reactive-api}/org/jboss/resteasy/reactive/server/spi/ServerMessageBodyWriter.html)
instead of [`MessageBodyWriter`]({jaxrsapi}/jakarta/ws/rs/ext/MessageBodyWriter.html)
...
[`Consumes`]({jaxrsapi}/jakarta/ws/rs/Consumes.html)/[`Produces`]({jaxrsapi}/jakarta/ws/rs/Produces.html) annotations
...
extending the [`ReaderInterceptor`]({jaxrsapi}/jakarta/ws/rs/ext/ReaderInterceptor.html) or
[`WriterInterceptor`]({jaxrsapi}/jakarta/ws/rs/ext/WriterInterceptor.html) on a class annotated with
[`@Provider`]({jaxrsapi}/jakarta/ws/rs/ext/Provider.html).
...
In Quarkus, the Quarkus REST extension and [the REST Client extension](rest-client.adoc) share the same infrastructure.
...
All <<request-parameters,Request Parameters>> can be declared as [`String`]({jdkapi}/java/lang/String.html),
but also as any of the following types:

1. Types for which a [`ParamConverter`]({jaxrsapi}/jakarta/ws/rs/ext/ParamConverter.html) is available via a registered
[`ParamConverterProvider`]({jaxrsapi}/jakarta/ws/rs/ext/ParamConverterProvider.html).
2. Primitive types.
3. Types that have a constructor that accepts a single [`String`]({jdkapi}/java/lang/String.html) argument.
4. Types that have a static method named `valueOf` or `fromString` with a single [`String`]({jdkapi}/java/lang/String.html) argument
...
5. [`List<T>`]({jdkapi}/java/util/List.html), [`Set<T>`]({jdkapi}/java/util/Set.html), or
[`SortedSet<T>`]({jdkapi}/java/util/SortedSet.html), where `T` satisfies 1, 3 or 4 above.
...
[`Request`]({jaxrsapi}/jakarta/ws/rs/core/Request.html) context object:
...
[`Accept`]({httpspec}#section-5.3.2) header, in the case of JSON:
...
TIP: See the [HTTP reference guide](http-reference.adoc#http-compression) for the full list of compression configuration options including minimum response size, compression level, Brotli, and request decompression.
...
See the [REST Client Guide](rest-client.adoc) for more information about the REST client.
