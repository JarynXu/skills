...
> Upstream path: `docs/src/main/asciidoc/rest-virtual-threads.adoc`  
> Upstream Git blob: `927682f03ce9feb19b56534270e69ad995424ba1`  
...
Check the [virtual thread reference guide](./virtual-threads.adoc#why-not) for details.
====
...
It has been removed in Java 24+. For Java 24+, use the JFR events or Quarkus testing extensions instead (see the [Testing virtual thread applications](./virtual-threads.adoc#testing-virtual-thread-applications) section).
...
The `-Djdk.tracePinnedThreads` will detect pinned carrier threads while running tests on Java 21-23 (See [the virtual thread reference guide for details](./virtual-threads.adoc#pinning)).
...
Read more about the rest client in the dedicated [guide](./rest-client.adoc).
...
That's what we call _pinning_ (more info in [the virtual thread reference guide](./virtual-threads.adoc#pinning)).

IMPORTANT: This flag **only works on Java 21-23** and has been removed in Java 24+. For Java 24+, use the Quarkus `junit-virtual-threads` extension described in [the virtual thread reference guide](./virtual-threads.adoc#testing-virtual-thread-applications), which uses JFR events to detect pinning across all Java versions.

NOTE: Starting with Java 24, thanks to [JEP 491](https://openjdk.org/jeps/491), `synchronized` blocks no longer cause pinning.
...
- [@RunOnVirtualThread in messaging applications](./messaging-virtual-threads.adoc) (this guide covers Apache Kafka)
- [@RunOnVirtualThread in gRPC services](./grpc-virtual-threads.adoc)
- [the virtual thread reference guide](./virtual-threads.adoc) (include native compilation and containerization)
