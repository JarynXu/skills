...
> Upstream path: `docs/src/main/asciidoc/transaction.adoc`  
> Upstream Git blob: `4ab57159d06684d58727f100f4dfbebfabe3b809`  
...
The `quarkus-narayana-jta` extension provides a Transaction Manager that coordinates and exposes transactions to your applications as described in the [Jakarta Transactions](https://jakarta.ee/specifications/transactions/) specification, formerly known as Java Transaction API (JTA).
...
This means that models that propagate transaction context, such as [Java Transaction Service](https://download.oracle.com/otndocs/jcp/7309-jts-1.0-spec-oth-JSpec/) (JTS), REST-AT, WS-Atomic Transaction, and others, are not supported by the `narayana-jta` extension.
...
[Context Propagation guide](context-propagation.adoc).
...
Just assign the transaction [scope](cdi.adoc#bean-scope) to such beans using the `@TransactionScoped` annotation:
...
If no value is provided for the `datasource` property, Quarkus uses the [default datasource](datasource.adoc#configure-datasources).
...
For more configuration information, see the *Narayana JTA - Transaction manager* section of the Quarkus [All configuration options](all-config.adoc) reference.
...
[NOTE]
====
To work around the current known issue of [Agroal having a different view on running transaction checks](https://issues.redhat.com/browse/AG-209), set the datasource transaction type for the datasource responsible for writing the transaction logs to `disabled`:
