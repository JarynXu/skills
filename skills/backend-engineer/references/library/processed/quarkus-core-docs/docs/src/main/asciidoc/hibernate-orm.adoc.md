...
> Upstream path: `docs/src/main/asciidoc/hibernate-orm.adoc`  
> Upstream Git blob: `39c3bd2b0bb61c040c5987e2f20c81c0456c5290`  
...
The solution is located in the `hibernate-orm-quickstart` [directory]({quickstarts-tree-url}/hibernate-orm-quickstart).
...
* your JDBC driver extension; the following options are available:
  - `quarkus-jdbc-db2` for [IBM Db2](https://www.ibm.com/products/db2-database)
  - `quarkus-jdbc-h2` for [H2](https://www.h2database.com/html/main.html)
  - `quarkus-jdbc-mariadb` for [MariaDB](https://mariadb.com/)
  - `quarkus-jdbc-mssql` for [Microsoft SQL Server](https://www.microsoft.com/en-gb/sql-server/)
  - `quarkus-jdbc-mysql` for [MySQL](https://www.mysql.com/)
  - `quarkus-jdbc-oracle` for [Oracle Database](https://www.oracle.com/database/)
  - `quarkus-jdbc-postgresql` for [PostgreSQL](https://www.postgresql.org/)
...
```
<1> [Configure the datasource](datasource.adoc) for production, relying on [Dev Services](datasource#dev-services) for connection information in tests / dev mode.
...
A `SessionFactory` will be created based on the Quarkus `datasource` configuration as long as the Hibernate ORM extension is listed among your project dependencies.
...
You can then happily inject your `Session`:
...
TIP: If you prefer to stick to the Jakarta Persistence API, you can <<injecting-entry-points,inject an `EntityManager` instead of a `Session`>>.
...
For [supported databases](datasource.adoc#extensions-and-database-drivers-reference),
the [Hibernate ORM dialect]({hibernate-orm-docs-url}#database-dialect)
...
* IBM Db2 {db2-default-version}
* MariaDB {mariadb-default-version}
* Microsoft SQL Server {mssql-default-version}
* MySQL {mysql-default-version}
* Oracle Database {oracle-default-version}
* PostgreSQL {postgres-default-version}

These defaults enable all available features and best performance when using these versions.
They also match the [Dev Services](databases-dev-services.adoc) container versions.
...
.`{config-file}` with an explicit (older) `db-version`
```properties
...
[NOTE]
====
As described above, the version can either be set explicitly via a `quarkus.datasource.db-version` configuration property,
or defaulted to the latest stable version.
Quarkus will check the configured version against the actual database version on startup,
leading to a startup failure when the actual version is lower than the configured version.

This is a safeguard: for databases older than what is configured,
...
If you know the database won't be reachable on startup, consider configuring <<offline,offline startup>>,
which will automatically disable the check and warning.

As a last resort, you can disable the version check completely
...
If [your database does not have a corresponding Quarkus extension](datasource.adoc#other-databases),
or if the defaults do not match your needs for some reason,
you will need to set the [Hibernate ORM dialect]({hibernate-orm-docs-url}#database-dialect) explicitly:
...
in the [official list of dialects]({hibernate-orm-dialect-docs-url}), *without* the `Dialect` suffix,
...
There are various optional properties useful to refine your `SessionFactory` or guide guesses of Quarkus.
...
contact us on the [quarkus-dev mailing list]({quarkus-mailing-list-index}).
====
...
If you are familiar with using Hibernate ORM in Quarkus, you probably already have injected the `Session` using CDI:
...
This will inject the `Session` of the default persistence unit.

Injecting the `Session` of a named persistence unit (`users` in our example) is as simple as:
...
If you prefer to stick to the Jakarta Persistence API, you can inject an `EntityManager` instead, using the exact same mechanism:
...
EntityManager entityManagerForUsers;
```

[NOTE]
====
The injected `Session` or `EntityManager` instance is a proxy that requires an active transaction for interaction.
...
You can inject the `SessionFactory` (or its Jakarta Persistence equivalent, `EntityManagerFactory`) of a named persistence unit using the exact same mechanism:
...
In addition to `Session` and `SessionFactory` (and their Jakarta Persistence equivalents `EntityManager` and `EntityManagerFactory`), Quarkus also supports injecting the following JPA/Hibernate components:
...
The Quarkus extension for Hibernate ORM supports injecting [attribute converters]({hibernate-orm-docs-url}#basic-jpa-convert) and [entity listeners]({hibernate-orm-docs-url}#events-jpa-callbacks) into Hibernate ORM.
...
When a persistence unit is configured at build time, and it is assigned entity types or an [active datasource](datasource.adoc#datasource-active), the persistence unit is active by default.
...
to [use one of a pre-determined set of datasources at runtime](datasource.adoc#datasource-active).
...
[Setting](config-reference.adoc#configuration-sources) `quarkus.hibernate-orm."pg".active=true` and `quarkus.datasource."pg".active=true` at runtime
...
[TIP]
====
[Custom configuration profiles](config-reference.adoc#custom-profiles) can help simplify such a setup.
...
[setting `quarkus.profile`](config-reference.adoc#multiple-profiles):
...
To achieve this, inject an `InjectableInstance<SessionFactory>` or `InjectableInstance<Session>` with an `@Any` qualifier and call [`getActive()`](cdi-integration.adoc#inactive-synthetic-beans).
...
Alternatively, you can define a [CDI bean producer](cdi.adoc#ok-you-said-that-there-are-several-kinds-of-beans) for the beans of the default persistence unit.
...
* Persistence units defined in `persistence.xml` always use the [default datasource](datasource.adoc#configure-a-single-datasource).
...
[`hibernate.dialect`]({hibernate-orm-docs-url}#settings-hibernate.dialect)/[`jakarta.persistence.database-product-name`]({hibernate-orm-docs-url}#settings-jakarta.persistence.database-product-name)
and possibly [`jakarta.persistence.database-product-version`]({hibernate-orm-docs-url}#settings-jakarta.persistence.database-product-version).
...
so in this case the appropriate reference is the [documentation on hibernate.org]({hibernate-orm-docs-url}#configurations).
...
* in `application.properties` through the (build-time) [`quarkus.hibernate-orm.mapping-files`](#quarkus-hibernate-orm_quarkus-hibernate-orm-mapping-files) property.
...
During tests and in [development mode](dev-mode-differences.adoc), Hibernate ORM benefits from [datasource Dev Services](datasource.adoc#dev-services),
...
In fact, this is the default when using [Dev Services](dev-services.adoc).
+
[TIP]
====
...
Or if you use a database schema migration tool like [Flyway](flyway.adoc) or [Liquibase](liquibase.adoc).
+
...
You can define different [configuration profiles](config-reference.adoc#profiles)
...
If you have the [Flyway extension](flyway.adoc) installed when running in development mode,
...
You can rely on [Flyway](flyway.adoc), [Liquibase](liquibase.adoc) or custom setups to create/migrate your database schema,
...
For more information about Hibernate Envers, see [hibernate.org/orm/envers/](https://hibernate.org/orm/envers/).
...
You can find all information about Spatial in the Hibernate user guide: [Hibernate ORM Spatial]({hibernate-orm-docs-url}#spatial).
...
You can find all information about Vector in the Hibernate user guide: [Hibernate ORM Vector]({hibernate-orm-docs-url}#vector).

[[metrics]]
## Metrics
[Micrometer](telemetry-micrometer.adoc) is
...
All configuration is optional; for more details see [Using Transactions in Quarkus](transaction.adoc).
...
More details about this connection pool can be found in [Quarkus - Datasources](datasource.adoc).

Second Level Cache::
As explained earlier in the <<caching,Caching section>>, you don't need to pick an implementation.
A suitable implementation based on [JCache](https://jcp.org/en/jsr/detail?id=107) and [Caffeine](https://github.com/ben-manes/caffeine) is included as a transitive dependency of the Hibernate ORM extension, and automatically integrated during the build.
...
The [Hibernate ORM with Panache](hibernate-orm-panache.adoc) extension facilitates the usage of Hibernate ORM by providing active record style entities (and repositories) and focuses on making your entities trivial and fun to write in Quarkus.
...
Jump over to [Quarkus - Datasources](datasource.adoc) for all details.

[[multitenancy]]
## Multitenancy

"The term multitenancy, in general, is applied to software development to indicate an architecture in which a single running instance of an application simultaneously serves multiple clients (tenants). This is highly common in SaaS solutions. Isolating information (data, customizations, etc.) pertaining to the various tenants is a particular challenge in these systems. This includes the data owned by each tenant stored in the database" ([Hibernate User Guide]({hibernate-orm-docs-url}#multitenacy)).

Quarkus currently supports the [separate database]({hibernate-orm-docs-url}#multitenacy-separate-database) approach, the [separate schema]({hibernate-orm-docs-url}#multitenacy-separate-schema) approach and the [discriminator]({hibernate-orm-docs-url}#multitenacy-discriminator) approach.

To see multitenancy in action, you can check out the [hibernate-orm-multi-tenancy-schema-quickstart]({quickstarts-tree-url}/hibernate-orm-multi-tenancy-schema-quickstart) or [hibernate-orm-multi-tenancy-database-quickstart]({quickstarts-tree-url}/hibernate-orm-multi-tenancy-database-quickstart).
...
public class FruitResource {
    ...
    public Fruit[] getFruits() {
        return session.createNamedQuery("Fruits.findAll", Fruit.class)
...
[NOTE]
====
If you also use [OIDC multitenancy](security-openid-connect-multitenancy.adoc) and both OIDC and Hibernate ORM tenant IDs are the same,
...
The following setup will use the [Flyway](flyway.adoc) extension to achieve this goal.
...
<3> [Configure the datasource](datasource.adoc).
<4> Configure [Flyway](flyway.adoc) for database initialization,
...
<4> [Configure the datasource](datasource.adoc) for one tenant, `base`.
<5> Configure [Flyway](flyway.adoc) for database initialization for tenant `base`,
because schema generation by Hibernate ORM is not supported in this case.
<6> [Configure the datasource](datasource.adoc) for another tenant.
+
There could be more tenants, but here we're stopping at two.
<7> Configure [Flyway](flyway.adoc) for database initialization for tenant `mycompany`,
because schema generation by Hibernate ORM is not supported in this case.
...
<2> [Configure the datasource](datasource.adoc).
...
This implies all these integrations, including those from other Quarkus modules (e.g. [SmallRye Health](smallrye-health.adoc)) must be done manually.
====
...
You can assign an [`org.hibernate.Interceptor`]({hibernate-orm-docs-url}#events-interceptors)
...
In order to create one interceptor instance per session instead,
...
In the Hibernate Validator documentation, you can find [the list of available constraints]({hibernate-validator-docs-url}#section-builtin-constraints) and their impact on the DDL generation.
...
explicitly handling these exceptions might be redundant. See the [REST end point validation](validation.adoc#rest-end-point-validation)
...
For a more detailed overview of static metamodel, please refer to the [Jakarta Persistence specification]({jakarta-persistence-spec-url}#a6072).
