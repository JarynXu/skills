...
> Upstream path: `docs/src/main/asciidoc/getting-started-testing.adoc`  
> Upstream Git blob: `5a94c4690c484babab556efe157760d4abc9d059`  
...
* The completed greeter application from the [Getting Started Guide](getting-started.adoc)
...
NOTE: Quarkus supports Continuous testing, but this is covered by the [Continuous Testing Guide](continuous-testing.adoc).
...
The solution is located in the `getting-started-testing` [directory]({quickstarts-tree-url}/getting-started-testing).
...
[JUnit User Guide](https://docs.junit.org/current/writing-tests/test-execution-order.html#classes).
...
Building on the features provided by `InjectMock`, Quarkus also allows users to effortlessly take advantage of [Mockito](https://site.mockito.org/) for spying on the beans supported by `QuarkusMock`.
...
If you are using the `quarkus-hibernate-orm-panache` or `quarkus-mongodb-panache` extensions, check out the [Hibernate ORM with Panache Mocking](hibernate-orm-panache.adoc#mocking) and [MongoDB with Panache Mocking](mongodb-panache.adoc#mocking) documentation for the easiest way to mock your data access.

## Testing Security

If you are using Quarkus Security, check out the [Testing Security](security-testing.adoc) section for information on how to easily test security features of the application.
...
This is covered in the [Native Executable Guide](building-native-image.adoc).
...
More information can be found in the [Testing the native executable Guide](building-native-image.adoc#testing-the-native-executable).
...
This means that `@QuarkusIntegrationTest` works out of the box with containers launched via [Dev Services](dev-services.adoc), but it also means that it enables using <<quarkus-test-resource,QuarkusTestLifecycleManager>> resources that launch additional containers.
...
* `java.util.logging.manager` (see [Logging Guide](logging.adoc))

* `maven.home` (only if there are any custom settings in `${maven.home}/conf/settings.xml`, see [Maven Guide](maven-tooling.adoc))
...
By default, tests should just work with [Dev Services](dev-services.adoc), however from some use cases you may need access to
...
You can find more examples and hints in the [testing components reference guide](testing-components.adoc).
