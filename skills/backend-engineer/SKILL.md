...
4. **Choose evidence before mutation, and choose the implementation shape with it.** Decide both what structure represents the behavior with the least accidental complexity and what evidence can falsify it. Prefer direct code for simple behavior; introduce abstraction only for a stable responsibility, real consumer, substitution boundary, or quality/failure need. Risk chooses verification depth; fashion and convenience do not choose architecture.
