---
name: code-review
description: Independently review changed code for structural complexity, ownership mistakes, abstraction quality, maintainability, clarity, consistency, correctness risks, and behavior-preserving simplification. Use for code review, deep structural review, maintainability review, implementation refinement, or a second-pass review after code is implemented and verified.
---
...
Treat this skill as the repository's general independent code-review capability. Structural complexity and refinement are the initial focus, but future code-quality review concerns should extend this skill rather than fragmenting into narrowly named review skills unless they require a genuinely different professional boundary.
...
A review-driven improvement must not silently alter product semantics, public contracts, error behavior, side-effect ordering, persistence rules, concurrency semantics, authorization, observability, or compatibility.
...
If material findings exist, lead with them in consequence order and distinguish blocking structural issues from optional refinements. If none are found, state that no material code-review finding was found within the reviewed evidence and name meaningful unverified boundaries.
